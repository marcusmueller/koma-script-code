...
wird \Length{textheight}\IndexLength{textheight} auch den Wert von
\Length{textwidth} gesetzt. Umgekehrt wird jedoch \Length{textwidth} nicht auf
den vorherige Wert von \Length{textheight} gesetzt. Das ist
inkonsistent. Meines Wissens wird \Length{textwidth} nicht geändert, weil
andere Pakete oder Anwenderanweisungen gestört werden können, wenn man es auf
\Length{textheight} setzen würde. Jedoch hat auch die Änderung von
\Length{textwidth} diese Potential und in der Tat beschädigt dies die Funktion
beispielsweise der Pakete \Package{showframe}\IndexPackage{showframe} und
...
deaktiviert wird. Es ist zu beachten\textnote{Achtung!}, dass eine
nachträgliche Zuweisung an Option \Option{lscape} mit
\Macro{KOMAoption}\IndexCmd{KOMAoption} oder
\Macro{KOMAoption}\IndexCmd{KOMAoptions} nur eine Wirkung hat, wenn sie
