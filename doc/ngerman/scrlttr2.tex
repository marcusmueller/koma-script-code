...
geändert werden. Die Option versteht als die
...
%
\EndIndexGroup
\vskip -1\ht\strutbox plus .75\strutbox % Wegen Beispiel am Ende der Erklärung
...
\DescRef{\LabelBase.option.refline}\IndexOption{refline}%
\important{\DescRef{\LabelBase.option.refline}} werden von der
Datumszeile beachtet. Voreingestellt\textnote{Voreinstellung} ist rechtbündig.%
\fi
...
Betreff abweichend von der Voreinstellung
\OptionValue{subject}{beforeopening}%
\important{\OptionValue{subject}{beforeopening}\\
  \OptionValue{subject}{underlined}\\
  \OptionValue{subject}{centered}\\
  \OptionValue{subject}{right}} erst nach der Anrede gesetzt werden soll. Eine
andere Formatierung kann mit \OptionValue{subject}{underlined} und
...
\autoref{tab:\LabelBase.subject} zu entnehmen. Es wird
ausdrücklich\textnote{Achtung!} darauf hingewiesen, dass bei der Einstellung
...
\begin{Example}
  ...
  \Macro{raggedsignature} um:\xmpllabel{letterwithoutlco}%
...
\hyperref[\LabelBase.pagestyle.plain.letter]{\PageStyle{plain}}-Seitenstils
...
unterstützt \Class{scrlttr2} dies nicht aktiv. Daher gibt es auch nicht die
...
\Class{scrbook}, \Class{scrreprt} und \Class{scrartcl} dokumentiert
sind. Bei \Package{scrletter} ist für dergleichen ohnehin die verwendete
Klasse zuständig.%
\iffalse% Umbruchkorrekturtext
  \par
...
\begin{Example}
  ...
  \PageRefxmpl{letterwithoutlco} erheblich zusammen:
