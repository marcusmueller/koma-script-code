...
\begin{desclist}
  \desccaption{Von \Class{scrlttr2} und \Package{scrletter} unterstützte
    Variablen\label{tab:\LabelBase.variables}} {Von \Class{scrlttr2} und
    \Package{scrletter} unterstützte Variablen (\emph{Fortsetzung})}
...
Haken\Index{Haken}. Die Klasse \Class{scrlttr2} und das Paket
\Package{scrletter} fügen zwei weitere Haken hinzu, die mit
...
\end{Example}
\else%
Wie in \cite{latex:clsguide} erwähnt, gibt es bei \LaTeX{} die Möglichkeit,
während des \LaTeX-Laufs eines Dokuments zusätzliche \PName{Anweisungen}
ausführen zu lassen. Zu diesem Zweck stellt der \LaTeX-Kern die Anweisungen
\Macro{AtEndOfClass}\IndexCmd{AtEndOfClass} und
\Macro{AtBeginDocument}\IndexCmd{AtBeginDocument} zur Verfügung. Man nennt
solche Eingriffspunkte auch \emph{hooks}\Index{hook=\emph{hook}}, also
Haken\Index{Haken}. \KOMAScript{} fügt zwei weitere Haken hinzu, die mit
\Macro{AtBeginLetter} und
\Macro{AtEndLetter}\ChangedAt{v2.95}{\Class{scrlttr2}} mit Inhalt versehen
werden können. Wie man schon daran erkennt, dass die \LaTeX-Kern-Anweisungen
für Haken nicht in \cite{latex:usrguide} sondern in \cite{latex:clsguide}
...
%  \caption
  \begin{captionbeside}
    [{%
      ...
      Briefkopfs von \Class{scrlttr2} und \Package{scrletter}%
    }%
    [l]%
%    \label{tab:\LabelBase.fromrule}%
...
unterstützt \KOMAScript{} dies nicht aktiv. Daher gibt auch nicht die Optionen
...
\href{http://mirrors.ctan.org/macros/latex/contrib/koma-script/doc/lppl-de.txt}{\File{lppl-de.txt}}
im \KOMAScript-Paket). Wenn Sie zwar über die notwendigen Maße aus einer
bisher nicht unterstützen Briefnorm verfügen, sich jedoch nicht in der Lage
sehen, selbst eine passende \File{lco}-Datei zu erstellen, so können Sie sich
ebenfalls mit dem \KOMAScript-Autor, Markus Kohm, in Verbindung
...
Laden der \File{lco}-Datei erfolgt dann direkt nach der Klasse.

Empfohlen\textnote{Empfehlung!} wird jedoch nicht nur für \Package{scrletter},
sondern auch für die Klasse \Class{scrlttr2}, eine \File{lco}-Datei über
\Macro{LoadLetterOption} oder
\Macro{LoadLetterOptions}\ChangedAt{v3.14}{\Class{scrlttr2}} erst danach zu
laden. Dies ist auch zu einem späteren Zeitpunkt, selbst nach
\Macro{begin}\PParameter{document} und sogar innerhalb einer anderen
\File{lco}-Datei möglich.  Der \PName{Name} der \File{lco}-Datei wird in
diesen Fällen ebenfalls ohne Endung übergeben. Während als Argument von
\Macro{LoadLetterOption} der \PName{Name} von genau einer \File{lco}-Datei
...
der Einstellung
\OptionValueRef{\LabelBase}{refline}{wide}\IndexOption{refline~=\PValue{wide}}
(siehe \autoref{sec:\LabelBase.firstpage},
...
\end{lstcode}
  ...
  aus \cite{package:adrconv}. Eine Seite des Ergebnisses sieht dann etwa so
  aus:
  \begin{center}
...
%  LocalWords:  Standardklassen Briefpaket Dokumentteile Briefkontext
