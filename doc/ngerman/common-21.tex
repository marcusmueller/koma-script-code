...
Standardklassen die linke Marke hingegen gar nicht erst.
...
Seite zusammengestellt wurde. \LaTeX{} selbst verwendet dies selbst
...
\Macro{rightmark} nur den rechten Teil verändert, wird der linke Teil
(unverändert) mit gesetzt. Entsprechend setzen im doppelseitigen Layout die
höheren Gliederungsebenen beim Seitenstil
\Pagestyle{headings}\important{\Pagestyle{headings}}\IndexPagestyle{headings}
immer beiden Marken. Beispielsweise verwendet \Macro{chaptermark} dann
...
\Macro{section} eine neue rechte Marke gesetzt wurde.
...
sein. Dagegen spielt es keine Rolle, ob sie im Hintergrund oder im Vordergrund
ausgegeben werden, da alle Ebenen erst nach der Zusammenstellung der aktuellen
