...
Marginalien. Eine\ChangedAt{v3.12}{\Package{typearea}} eventuell
...
\autoref{cha:scrpage} oder \autoref{cha:scrlayer-scrpage}).
...
Es ist zu beachten\textnote{Achtung!}, dass \LaTeX{} im optionalen Argument
von \Macro{documentclass} oder \Macro{usepackage} die Verwendung von
\LaTeX-Längen wie \Length{baselineskip} nicht unterstützt, sondern mit einer
Fehlermeldung quitiert. Das ist auch der Grund, warum neben
\Option{heightheight} auch noch \Option{headlines} implementiert wurde.
...
\Length{footheight}\IndexLength[mainindex]{footheight}, falls diese noch nicht
existiert. Ob diese dann auch beispielsweise von Klassen und Paketen für die
Gestaltung von Kopf und Fuß verwendet wird, hängt von den verwendeten Paketen
ab. Das \KOMAScript-Paket \Package{scrlayer-scrpage} berücksicht diese auf
jeden Fall und arbeitet somit aktiv mit \Package{typearea} zusammen. Die
\KOMAScript-Klassen berücksichten \Length{footheight} hingegen nicht, da sie
ohne Paketunterstützung nur Seitenstile mit einzeilige Seitenfüße
anbieten.

Die Option \Option{footlines}\important{\Option{footlines}} setzt man
vergleichbar zu \Option{headlines} auf die Anzahl der Kopfzeilen.
...
\begin{Example}
  ...
  vorgesehenen Höhe führt aber wahrscheinlich zu einer unausgeglichenen
  Verteilung von Satzspiegeln und Rändern. Außerdem würde beispielsweise das
  Paket \Package{scrlayer-scrpage}, mit dem ein solcher Fußinhalt gesetzt
...
\LaTeX-Längen wie \Length{baselineskip} nicht unterstützt, sondern mit einer
Fehlermeldung quitiert. Das ist auch der Grund, warum neben
\Option{footheight} auch noch \Option{footlines} implementiert wurde.
