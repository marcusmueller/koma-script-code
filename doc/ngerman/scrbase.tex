...
sind. Genau das ist mit Hilfe von \Macro{AtEndOfFamilyOptions} und dessen
Sternvariante\ChangedAt{v3.23}{\Package{scrbase}} möglich. Die Rückmeldung von
...
\end{Declaration}
\iffalse% Lange veraltet --> Doku entfernt.
...
von \Macro{FamilyElseValue} ebenfalls überholt. %
\else%
Diese\ChangedAt{v3.12}{\Package{scrbase}} Anweisung ist seit Version~3.12
veraltet. Ihre ursprüngliche Bedeutung ist nicht mehr von Belang. %
\fi%
Ihre Verwendung wird jedoch erkannt und führt dann zu einer Aufforderung, den
Code entsprechend anzupassen.%
\EndIndexGroup
...
angeben. Insbesondere wenn keine \KOMAScript-Klasse verwendet wird, sei der
zweite Weg unter Verwendung von \Macro{PassOptionsToPackage} ausdrücklich
empfohlen.
\begin{Example}
  Der Anwender will nicht, dass die Anweisungen
  \DescRef{\LabelBase.cmd.ifpdftex} und
  \DescRef{\LabelBase.cmd.ifVTeX} von \Package{scrbase} definiert
  werden. Also verwendet er beim Laden der Klasse:
\begin{lstcode}
  \documentclass[%
    internalonly=/ifpdftex/ifVTeX%
  ]{foo}
\end{lstcode}
  ...
  und \DescRef{\LabelBase.cmd.ifpdftex} werden im weiteren Verlauf
  dieses Abschnittes erklärt.

  Falls \Class{foo} keine \KOMAScript-Klasse ist, kann man in der
  \File{log}-Datei des \LaTeX-Laufs eventuell die Warnung
\begin{lstoutput}
  LaTeX Warning: Unused global option(s):
       [internalonly=/ifpdftex/ifVTeX].
\end{lstoutput}
  finden. Diese Warnung trifft trotz Verwendung von \Package{scrbase} und
  trotz der darin stattfindenden Auswertung der Option auf, wenn
  eine Klasse \Package{scrbase} vor der Verarbeitung der eigenen Optionen
  lädt. Dies dürfte die Regel sein, da nur so die Klasse \Package{scrbase} für
  die Definition eigener Optionen verwenden kann. Daher sei Anwendern das
  Setzen der Option per \Macro{PassOptionsToPackage} vor dem Laden der Klasse
  empfohlen. Mit
\begin{lstcode}
...
Das\textnote{Achtung!} \LaTeX-Team hat 2019 in Version~1.0 des Pakets
\Package{iftex}\IndexPackage{iftex}\important{\Package{iftex}} selbst
\DescRef{\LabelBase.cmd.ifpdftex} und \DescRef{\LabelBase.cmd.ifVTeX} neu
definiert. Leider sind diese Definitionen inkompatibel mit den bereits seit
2002 existierenden Definitionen in \Package{scrbase}. Diese Inkompatibilität\important[i]{\Package{ifluatex}\\ \Package{ifpdf}\\
  \Package{ifvtex}\\ \Package{ifxetex}} betrifft leider auch die Pakete
\Package{ifpdf}\IndexPackage{ifpdf} und \Package{ifvtex}\IndexPackage{ifvtex}
und sogar von \Package{ifxetex}\IndexPackage{ifxetex} und
\Package{ifluatex}\IndexPackage{ifluatex} in ihren Neuauflagen als
Wrapper-Pakete von \Package{iftex}. Da anzunehmen ist, dass sich Paketautoren
und Beispiele im Internet zunehmend auf die Implementierung in \Package{iftex}
verlassen, ist leider zu befürchten, dass solche Pakete und Beispiele
zunehmend inkompatibel mit \Package{scrbase} werden.  Um solche Probleme zu
vermeiden, muss leider empfohlen werden, die im Beispiel gezeigte Option
grundsätzlich zu setzen und so auf die \Package{scrbase}-Version der genannten
Befehle zu verzichten. Eine grundsätzliche Änderung in \Package{scrbase} würde
leider ihrerseits unvermeidbar zu Inkompatibilitäten mit existierenden
Dokumenten und eventuell auch mit Paketen oder Klassen führen, die sich auf
die bisherige Definition in \Package{scrbase} verlassen. Daher ist sie leider
nicht möglich.  Da nur wenige Anwender wissen, welche Pakete \Package{iftex},
\Package{ifpdf}, \Package{ifluatex}, \Package{ifvtex} oder \Package{ifxetex}
verwenden, bleibt für neue Dokumente somit der gezeigte, eher unkomfortable
Weg. Für existierende Dokumente kann nicht ausgeschlossen werden, dass diese
im Zuge von Updates abhängiger Pakete irgendwann Fehler produzieren und
geändert werden müssen. Der Autor von \KOMAScript{} bedauert dies
ausdrücklich, zumal solche Probleme durch die freundliche Rücksichtnahme
früherer Autoren der genannten Pakete über Jahrzehnte vermieden werden
konnten.%
\EndIndexGroup
...
\Macro{@ifundefined} wird auch bei älteren \LaTeX-Versionen %
...
\fi%
In der Regel sollte man besser auf die gewünschte Anweisung oder Fähigkeit
testen.%
\iffalse% Umbruchvarianten
Von\textnote{Achtung!} dieser Anweisung gibt es kein
Benutzeräquivalent.%
\fi%
\EndIndexGroup
...
Beachten\textnote{Achtung!} Sie auch den Hinweis bezüglich \Package{iftex} am
Ende der Erklärung zu Option \DescRef{\LabelBase.option.internalonly}.%
\EndIndexGroup
...
sinnvoll nutzen, %
\else%
In der Dokumentpräambel enspricht \Macro{if@atdocument}
\iftrue dem Schalter \fi
\Macro{iffalse}, nach \Macro{begin}\PParameter{document} hingegen
...
verglichen. Sind sie gleich, so wird der \PName{Dann-Teil} ausgeführt,
anderenfalls der \PName{Sonst-Teil}.%
\iffalse% Umbruchkorrektur
\iftrue% Umbruchvarianten
\ Von dieser Anweisung gibt es keine interne Variante.%
...
der Name. Der verbleibende Rest ohne dem führenden Querstrich ist das
