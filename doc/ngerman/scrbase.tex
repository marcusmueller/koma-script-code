...
\end{lstcode}
  ...
  kann vermieden werden, wenn vor der letzten Zeile für das Paket selbst
  ebenfalls einen entsprechenden Schlüssel definiert:
\begin{lstcode}
  \DefineFamilyKey{Fleischermeister}%
                  {Aufschnitt}[true]{}%
\end{lstcode}
...
\begin{lstcode}
  ...
  \DefineFamilyKey{Fleischermeister}%
                  {Aufschnitt}[true]{%
    \AtBeginDocument{\FamilyOptions{Aufschnitt=#1}}%
  }%
\end{lstcode}
  Somit wird während \Macro{begin}\PParameter{document} zunächst die Option so
