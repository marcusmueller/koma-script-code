...
\item[An welcher Position soll die Ebene ausgegeben
  ...
  Zur Beantwortung dieser Ebene dienen Eigenschaften zur Festlegung der
...
\item[Wie groß ist die Ebene?\textnote{horizontale und vertikale Größe}]%
  ...
  Position der Ebene Eigenschaften. Damit kann eine Ebene auch kleiner oder
...
\item[Ist die Ebene für Textausgabe oder für Grafik vorgesehen?\textnote{Text
  ...
  bei der Grafikausgabe eher davon ausgeht, dass der Ursprung in der linken
  unteren Ecke der Ebene liegt, wäre dies bei der Textausgabe eher ungünstig.
  Daher liegt der Ursprung für Textebenen um die Höhe einer Standardtextzeile
  unterhalb der oberen, linken Ecke der Ebene. Grafikebenen wiederum spannen
  von sich aus bereits eine \Environment{picture}-Umgebung\IndexEnv{picture}
  auf, in der zusätzliche Befehle zur Positionierung zur Verfügung stehen.
\item[Soll die Ebene auf linken oder rechten Seiten eines Dokuments gedruckt
  ...
  ungerade Seiten.
\item[Soll die Ebene in einseitigen oder in doppelseitigen Dokument verwendet
  werden?\textnote{einseitig oder doppelseitig}]%
...
\item[Welchen Inhalt hat die Ebene?\textnote{Inhalt}]%
  Die zugehörige Eigenschaft gilt schlicht an, was gedruckt werden soll, wann
...
weitere Eigenschaften kennen lernen, die jedoch auf diese primäre
...
Längen-Werte in Vielfache von \Macro{unitlength} um und kann daher an Stelle
von Koordinaten oder anderen von \Macro{unitlength} abhängigen Werten einer
...
\Macro{putLL}, \Macro{putLR} und \Macro{putC}. Die Anweisung wird nur
...
\end{document}
\end{lstcode}
Wie Sie sehen werden, passt die von \Package{typearea} vorgenommene Abbildung
auf einen ganzzahligen \PName{DIV}-Wert im Beispiel sehr gut.
\end{Example}
...
\Macro{DeclareNewPageStyleByLayers} wird leider erst in
