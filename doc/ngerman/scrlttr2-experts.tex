...
\DescPageRef{scrlttr2-experts.cmd.@setplength}) in eigenen
...
\autoref{sec:scrlttr2.firstpage},
\DescPageRef{scrlttr2.option.addrfield}) und \Option{backaddress} (siehe
\autoref{sec:scrlttr2.firstpage},
...
\DescPageRef{scrlttr2.option.version}) wird außer bei \Option{KOMAold}
...
\DescRef{maincls.cmd.markboth}\IndexCmd{markboth},
\DescRef{maincls.cmd.markright}\IndexCmd{markright} oder ähnlichen Anweisungen
...
\DescRef{maincls.cmd.markboth} oder \Macro{footnote} zu funktionieren.
%
\EndIndexGroup
%
\EndIndexGroup
...
die Variablen \Variable{nexthead} und \Variable{nextfoot} für
\PageStyle{letter} so verwendet, wie dies in \autoref{sec:scrlttr2.pagestyle}
für den Seitenstil \PageStyle{headings} dokumentiert ist. Der zugehörige
...
\end{labeling}%
\BeginIndex{FontElement}{field}\FontElementLabel{field}%
Mit Hilfe der Anweisungen \DescRef{scrlttr2.cmd.setkomafont} und \DescRef{scrlttr2.cmd.addtokomafont} (siehe
...
\BeginIndex{FontElement}{letter}\FontElementLabel{letter}%
...
\BeginIndex{FontElement}{measure}\FontElementLabel{measure}%
