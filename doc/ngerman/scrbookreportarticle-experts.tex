...
% \label{sec:maincls-experts.identification}
...
Mit Hilfe von \Macro{raggedchapterentry} besteht lediglich die Möglichkeit,
Kapiteleinträge im Inhaltsverzeichnis statt im voreingestellten Blocksatz im
linksbündigen Flattersatz zu setzen. Dazu ist die Anweisung als
...
\autopageref{desc:maincls-experts.cmd.raggedsection}. Voreingestellt ist
hingegen die leere Definition. Andere Einstellungen für
\Macro{raggedchapterentry} werden nicht empfohlen und können zu unerwarteten
Ergebnissen führen.%
%
...
% \label{sec:maincls-experts.fonts}
...
% \label{sec:maincls-experts.parskip}
...
% \subsection{Zähler}
% \label{sec:maincls-experts.counter}

\LoadNonFree{scrbookreportarticle-experts}{3}

% \subsection{Gliederung}
% \label{sec:maincls-experts.sections}
...
\item[\PValue{section}:] Stil einer Abschnittsüberschrift. Dieser Stil wird
  ...
  nicht unabhängig davon verändert werden.
\end{labeling}
...
\begin{table}
  ...
  \begin{tabularx}{\linewidth}{llX}
    ...
    \PValue{prefixfont} & \PName{Schrifteinstellung}  & 
                                          ...
                                          \FontElement{disposition} und dem
                                          Element des Gliederungsbefehls bei
                                          der Ausgabe einer Präfixzeile der
                                          Klassen \Class{scrbook} und
                                          \Class{scrreprt} oder der Nummer bei
                                          der Klasse \Class{scrartcl} in der
                                          Überschrift verwendet werden
                                          sollen. Hier sind alle
                                          Einstellungen erlaubt, die auch über
                                          \Macro{setkomafont} und
                                          \Macro{addtokomafont} für das
                                          Element der Präfixzeile oder der
                                          Nummer des Gliederungsbefehls
                                          erlaubt sind.\\
    \bottomrule
  \end{tabularx}
\end{table}
...
\KOMAScript~3.15 entsprechen:\IndexLength{@tempskipa}
\begin{lstcode}
...
\Macro{DeclareSectionCommand}-Einstellung \PValue{afterskip} ergibt. Werden
\Macro{chapterheadstartvskip} oder \Macro{chapterheadendvskip} umdefiniert und
sollen die Abstände weiterhin beispielsweise mit
\Macro{RedeclareSectionCommand} konfigurierbar sein, so sollte man in der
neuen Definition ebenfalls \Length{@tempskipa} verwenden.
...
Option nicht neu definiert. Ihre Original-Definitionen entsprechen bei
...
Da die Abstände über und unter den Überschriften wesentlich einfacher mit
\Macro{RedeclareSectionCommand} eingestellt werden können, wird nicht
empfohlen zu diesem Zweck stattdessen die hier beschriebenen Anweisungen
umzudefinieren. Sie sollten für tiefgreifendere Änderungen reserviert bleiben,
die nicht über \Macro{RedeclareSectionCommand} zu erreichen sind.
...
% \subsection{Anhang}
% \label{sec:maincls-expert.appendix}
...
% \label{sec:maincls-expert.bibliography}
...
%  LocalWords:  Zählername Spitzmarke
