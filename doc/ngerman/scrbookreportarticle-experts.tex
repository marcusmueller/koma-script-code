...
\begin{table}
  ...
  \begin{tabularx}{\linewidth}{llX}
    ...
    \PValue{font} & \PName{Schrifteinstellung}  & 
                                          ...
                                          \DescRef{maincls.cmd.setkomafont} und
                                          \DescRef{maincls.cmd.addtokomafont} für das
...
Für \DescRef{maincls.cmd.chapter} sind einige Einstellungen für die Überschriften von Option
...
Die Anweisung \DescRef{maincls.cmd.chapter} setzt die interne Länge \Length{@tempskipa} vor
Aufruf von \Macro{chapterheadstartvskip} automatisch auf den Wert, der sich
aus der
\DescRef{\LabelBase.cmd.DeclareSectionCommand}-Einstellung\IndexCmd{DeclareSectionCommand}
...
\DescRef{\LabelBase.cmd.DeclareSectionCommand}-Einstellung \PValue{afterskip} ergibt, und vor
dem Aufruf von \Macro{chapterheadmidvskip} mit dem Wert, der sich aus
\DescRef{\LabelBase.cmd.DeclareSectionCommand}-Einstellung \PValue{innerskip} ergibt. Werden
...
beispielsweise mit \DescRef{\LabelBase.cmd.RedeclareSectionCommand} konfigurierbar sein, so
sollte man in der neuen Definition ebenfalls \Length{@tempskipa} verwenden.

Da die Voreinstellungen für die Abstände von \DescRef{maincls.cmd.part} nicht von Option
\DescRef{maincls.option.headings} abhängen, werden auch die zugehörigen
Anweisungen von dieser Option nicht neu
