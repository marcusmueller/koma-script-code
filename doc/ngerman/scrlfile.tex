...
\ChangedAt{v3.32}{\Package{scrlfile}}Allerdings ergab sich in einem Dialog
mit dem \LaTeX-Team, dass dieses ein solches Vorgehen als überwiegend nicht
empfehlenswert betrachtet. Daher gelten sämtliche Anweisungen dieses
Abschnitts als veraltet und sollten nicht mehr verwendet werden. Bei
Verwendung von \LaTeX{} ab Version 2020-10-01 werden sie teilweise gar nicht
angeboten oder erzeugen eine Fehlermeldung.

\begin{Declaration}[0]
  \Macro{ReplaceInput}\Parameter{Dateiname}\Parameter{Ersatzdatei}
\end{Declaration}
Diese\ChangedAt{v3.32}{\Package{scrlfile}} Anweisung gilt bei Verwendung von
\LaTeX{} ab Version 2020-10-01 als veraltet, funktioniert derzeit jedoch
noch. Mit\ChangedAt{v2.96}{\Package{scrlfile}} ihr wird eine Ersetzung der
Datei mit dem als erstes angegebenen \PName{Dateiname} definiert. Wenn
...
\end{Declaration}
Diese\ChangedAt{v3.32}{\Package{scrlfile}} Anweisungen gelten bei Verwendung
von \LaTeX{} ab Version 2020-10-01 als veraltet und werden aufgrund eines
Problems auch nicht via Option \DescRef{\LabelBase.option.withdeprecated}
bereit gestellt. Bei Verwendung einer älteren \LaTeX-Version gelten die
Anweisungen ebenfalls als veraltet und werden voraussichtlich nur noch
...
\end{Example}
Eine Lösung dieser Probleme bestand bis Version 3.31 darin, statt
...
Da\ChangedAt{v3.32}{\Package{scrlfile}} die Anweisungen
\DescRef{\LabelBase.cmd.ReplaceInput}, \DescRef{\LabelBase.cmd.ReplaceClass}
und \DescRef{\LabelBase.cmd.ReplacePackage} als veraltet gelten, trifft dies
aber natürlich auch für die Befehle zur Aufhebung der Ersetzung zu. Sie
sollten daher ebenfalls nicht mehr verwendet werden.%
\EndIndexGroup
\end{Explain}
