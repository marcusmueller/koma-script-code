...
Es ist zu beachten, dass neben den in diesem Abschnitt dokumentieren
...
können. Siehe \autoref{sec:maincls.structure},
\autopageref{desc:maincls.cmd.DeclareSectionCommand}.
...
Diese\ChangedAt[2014/11]{v3.15}{\Class{scrbook}\and \Class{scrreprt}\and
  \Class{scrartcl}} Anweisungen dienen dazu einen neuen Gliederungsbefehl
\Macro{\PName{Name}} zu definieren oder den vorhandenen Gliederungsbefehl
\Macro{\PName{Name}} zu verändern. Dazu werden über das optionale Argument
...
sind \autoref{tab:maincls.declaresection.keys} zu entnehmen.
...
\begin{longtable}[l]{llp{\dimexpr\linewidth-6\tabcolsep-17em}}
  \caption[Eigenschaften bei der Deklaration von Gliederungsbefehlen]{Mögliche
    ...
    \label{tab:maincls.declaresection.keys}}\\
  \toprule
  ...
  \endlastfoot
    ...
    \PValue{beforeskip} & \PName{Länge} & Der Absolutwert gibt den vertikalen
                                          ...
                                          dass der Absatz nach der Überschrift
                                          ohne horizontalen Einzug gesetzt
                                          werden soll.\\
    ...
    \PValue{tocindent} & \PName{Länge}  & Der horizontale Einzug des zum
                                          ...
                                          erzeugt wird.\\ 
    \PValue{toclevel} & \PName{Ganzzahl}& Die Breite, die im zum
                                          Gliederungsbefehl gehörenden Eintrag
                                          ins Inhaltsverzeichnis für die
                                          Gliederungsnummer reserviert wird.\\
    \PValue{tocnumwidth} & \PName{Länge}& Die Ebene des zum Gliederungsbefehl
                                          gehörenden Eintrags ins
                                          Inhaltsverzeichnis, falls diese von
                                          \PValue{numwidth} abweichen soll
                                          (siehe auch \Counter{tocdepth} in \autoref{sec:maincls.toc},
                                          \autopageref{desc:maincls.counter.tocdepth}). \\
\end{longtable}
...
existierende Anweisung zu einem Gliederungsbefehl mit den angegeben
...
Gliederungsbefehl \Macro{\PName{Name}} unbedingt mit den angegeben
...
definiert werden können. Das ist der Stil, der von \KOMAScript{} für die
Überschriften von \Macro{section} bis \Macro{subparagraph} verwendet
wird. Überschriften im Stil von \Macro{part} und \Macro{chapter} können
darüber nicht definiert werden. Die Spezialüberschriften \Macro{addchap} und
\Macro{addsec} werden hingegen automatisch und mittelbar zusammen mit
\Macro{chapter} und \Macro{section} verändert. Sie sollten ebenfalls nicht
...
\end{lstcode}
  ...
  1\,sp angegeben. Der Grund ist einfach: Ein Wert von 0\,pt betrachtet
...
Dass im Beispiel tatsächlich nur die Schlüssel \PName{beforeskip} und
\PName{afterskip} verwendet werden mussten, liegt daran, dass seit
...
\Macro{paragraph} entspricht bei \Class{scrartcl} übrigens:
\begin{lstcode}
...
%  LocalWords:  Satzspiegels Schrifteinstellung
