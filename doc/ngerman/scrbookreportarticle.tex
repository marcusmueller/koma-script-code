...
\ProvidesFile{scrbookreportarticle.tex}[2013/10/08 KOMA-Script guide (chapter:
...
Bei Verwendung des Pakets \Package{scrlayer}\IndexPackage{scrlayer} (siehe
\autoref{cha:scrlayer}) muss man sich nicht die nummerischen Werte der
einzelnen Gliederungsebenen merken. Dann stehen dafür die Makros
\Macro{partlevel}\IndexCmd{partlevel},
\Macro{chapterlevel}\IndexCmd{chapterlevel},
\Macro{sectionlevel}\IndexCmd{sectionlevel} und so
weiter\IndexCmd{subsectionlevel}\IndexCmd{subsubsectionlevel}%
\IndexCmd{paragraphlevel} bis hinunter zu
\Macro{subparagraphlevel}\IndexCmd{subparagraphlevel} zur Verfügung.
\begin{Example}
...
automatischen und manuellen Kolumnentiteln bei Verwendung \iffree{des Pakets
  \Package{scrpage2}\important{\Package{scrpage2}} oder }{}des Pakets
...
\begin{Explain}
  ...
  Da\ChangedAt{v3.12}{\Class{scrbook}\and \Class{scrreprt}\and
    \Class{scrartcl}} \KOMAScript{} die zusätzlichen Gliederungsanweisungen
  ...
  \Counter{secnumdepth} auf -1 setzen. Dies sollte man beispielsweise bei der
  Umdefinierung von \Macro{chaptermark} und \Macro{sectionmark}
  berücksichtigen oder \Macro{addchapmark} und \Macro{addsecmark} ebenfalls
  passend umdefinieren.

  \OnlyAt{\Class{scrbook}\and \Class{scrreprt}}Während bei \Class{scrartcl}
  \Macro{chaptermark}, \Macro{addchapmark} und \Macro{chaptermarkformat} nicht
  existieren, gibt es die Anweisungen
  \OnlyAt{\Class{scrartcl}}\Macro{subsectionmark} und
  \Macro{subsectionmarkformat} nur bei \Class{scrartcl}. Dies ändert sich
  allerdings bei Verwendung des Pakets
  \Package{scrlayer-scrpage}\IndexPackage{scrlayer-scrpage}\iffree{ oder
    \Package{scrpage2}\IndexPackage{scrpage2}}{} (siehe
...
\IndexOption{bibliography~=\PValue{totocnumbered}} führt genau dazu,
