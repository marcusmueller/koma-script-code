...
\ProvidesFile{common-14.tex}[2013/05/29 KOMA-Script guide (common paragraphs)]
...
diese Einstellung wieder auf. Die Ränder\important{%
  \Macro{covertopmargin}\\
  \Macro{coverbottommargin}\\
  \Macro{coverleftmargin}\\
  \Macro{coverrightmargin}} dieser Umschlagseite werden über
\Macro{covertopmargin}\IndexCmd[indexmain]{covertopmargin} (oberer Rand),
\Macro{coverbottommargin}\IndexCmd[indexmain]{coverbottommargin} (unterer
Rand), \Macro{coverleftmargin}\IndexCmd[indexmain]{coverleftmargin} (linker
Rand) und \Macro{coverrightmargin}\IndexCmd[indexmain]{coverrightmargin}
(rechter Rand) bestimmt. Die Voreinstellungen sind von den Längen
...
\begin{Explain}
  ...
  Seit \KOMAScript~3.12 kann man allerdings die erste von \Macro{maketitle}
  ausgegebene Titelseite alternativ auch als Umschlagseite formatieren
  lassen. Dabei ändern sich nur die für diese Seite verwendeten
  Rändern. Näheres ist der Erklärung zu Option
  \OptionValue{titlepage}{firstiscover}%
  \IndexOption{titlepage~=\PValue{firstiscover}}
  auf \autopageref{desc:\csname label@base\endcsname.option.titlepage} zu
  entnehmen.
\end{Explain}
%
...
Schriftvoreinstellung für das Element
