...
\ProvidesFile{scraddr.tex}[2005/05/18 KOMA-Script guide (chapter: scraddr)]
...
\begin{Declaration}%
  ...
  \Parameter{Kommentar}\Parameter{Kürzel}\IndexCmd{adrentry}\\
  %
  \Macro{addrentry}\Parameter{Name}\Parameter{Vorname}\Parameter{Adresse}%
  \Parameter{Tel.}\Parameter{F1}\Parameter{F2}%
  \Parameter{F3}\Parameter{F4}\Parameter{Kürzel}\IndexCmd{addrentry}\\
  %
  \Macro{adrchar}\Parameter{Anfangsbuchstaben}\IndexCmd{adrchar}\\
  \Macro{addrchar}\Parameter{Anfangsbuchstaben}\IndexCmd{addrchar}%
\end{Declaration}%
