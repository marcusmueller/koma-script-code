...
% Date version of the translated file: 2013-10-24
...
Beside the features described in this chapter, \Package{scrlayer-scrpage}
provides several more, that could be of minor interest for the average user
and therefor is described from \autopageref{cha:scrlayer-scrpage-experts}
in \autoref{cha:scrlayer-scrpage-experts} of
\autoref{part:forExperts}. Nevertheless, if the below is not sufficient for
you in any case, please have a look at that chapter.
...
\Pagestyle{scrheadings}\IndexPagestyle{scrheadings} and
\Pagestyle{plain.scrheadings}\IndexPagestyle{plain.scrheadings}. Indeed
\Package{scrlayer-scrpage} provides to different default yet. You may select
one of those default manually using option \Option{pagestyleset}.

With \PName{setting} \PValue{KOMA-Script} the default will be used, that would
also be activated automatically if a \KOMAScript{} class has been
detected. Within double side layout thereby \Pagestyle{scrheadings} uses
...
Option \OptionValue{markcase}{used}\IndexOption{markcase~=used}. Pagestyle
\Pagestyle{plain.scrheadings} hasn't got running heads, but the page numbers
will be printed in the same kind.

With \PName{setting} \PValue{standard} the default will be used, that are
...
double side layout thereby \Pagestyle{scrheadings} uses running heads inner
...
\Pagestyle{scrheadings} is the same. But because of single side layout knows
...
\OptionValue{markcase}{upper}\IndexOption{markcase~=upper}. Within single side
layout page style \Pagestyle{plain.scrheadings} differs a lot from
...
\Pagestyle{plain.scrheadings} differs from \Pagestyle{plain} of the standard
classes. The standard classes would print it in the middle of the page
footer. But this wouldn't harmonise with the \Pagestyle{scrheadings}, so
\Pagestyle{plain.scrheadings} won't print a page number. But like
\Pagestyle{plain} it won't print a running head.
...
\Package{scrpage}, are defined for compatibility reasons only in
...
omit the colon. If you'd only use one value without colon, this will be the
\PName{width}.

For the \PName{width} as well a the \PName{offset} you can use any valid
...
\cite[section~3.5]{manual:eTeX} for more information about such expressions.
See \autoref{sec:scrlayer-scrpage.compatibilityOptions} for more information
about using, e.\,g., a \LaTeX{} length as an option value. The \PName{width}
can alternatively be one of the symbolic values shown in
\autoref{tab:scrlayer-scrpage.symbolic.values}.

The default \PName{width} is the width of the text area. The default
\PName{offset} depends on the used \PName{width}. In single side layout
generally the half if the difference of \PName{width} and the width of the
text area will be used. This results in horizontal centring the page head
above the text area. In difference to this, in double side layout generally a
third of the difference of \PName{offset} and the width of the text area will
be used. But if \PName{width} is the width of the whole text area plus the
marginal note column, default \PName{offset} will be zero. If you think, this
is complicated, you should simply use an explicit \PName{offset}.
%
\begin{table}
  \centering
  \caption[Symbolic values for options \Option{headwidth} and
  \Option{footwidth}]{Possible symbolisc values for the \PName{width} value of
  ...
  \begin{desctabular}
    ...
    \ventry{marginpar}{%
      the current width of the marginal note column inclusive the distance
    ...
    \ventry{textwithmarginpar}{%
      ...
      inclusive the distance between them (note: in this case and only in this
...
opposite to the colour wouldn't make sense and is not recommended for these
elements.%
...
Package \Package{scrpage2} has beside the options, that don't understand any
value assignment, also four commands
...
colour. Package \Package{scrlayer-scrpage} does also implement those
commands. Nevertheless these commands are deprecated and shouldn't be used any
longer. Let me also clarify, that these commands have never been made to
...
\Pagestyle{plain} page style will use the line settings made with the options
and commands described above. If the option is deactivated, the
\Pagestyle{plain} wont show the corresponding line.%
...
You've already been told, that the horizontal lines above or below the page
...
complies with option \Option{ilines}. Alternatively you can use option
