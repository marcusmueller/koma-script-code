...
\translator{Markus Kohm\and Elke Schubert\and Jana Schubert}
...
user interface, that is mostly compatible with \Package{scrpage2} and base on
\Package{scrlayer}. So if you are already familiar with \Package{scrpage2} and
don't use dirty tricks with \Package{scrpage2}, it should be easy for you to use
\Package{scrlayer-scrpage} instead of that package. Most examples to
\Package{scrpage2} in \LaTeX{} books or online resources should also work with
\Package{scrlayer-scrpage} at least with some minor code changes.
...
\autopageref{desc:maincls.cmd.chaptermarkformat}). Hence the usage of
...
and therefor are described from \autopageref{cha:scrlayer-scrpage-experts}
in \autoref{cha:scrlayer-scrpage-experts} of
\autoref{part:forExperts}. 
% [Todo note to editor: Improve following sentence:]
Nevertheless, if the below is not sufficient for you in any case, please have
...
with running heads. Nevertheless its more useful to fit the conventions
...
or other adaptions of classes (or packages) won't be necessary. So these page
...
However, these elements don't have their own font attributes that may be
...
\end{lstcode}
 ...
 \begin{lstcode}
  ...
  \end{document}
\end{lstcode}
  Now you'll also get a page header above the title head of the first
...
you should use the mark mechanism and ideally you should use it together with
the automatism described in the following section.%
\EndIndex{Cmd}{rohead}%
\EndIndex{Cmd}{cohead}%
\EndIndex{Cmd}{lohead}%
\EndIndex{Cmd}{rehead}%
...
the automatic running head as given by the words you've typed. This would be
...
if the option hasn't been used and \KOMAScript{} class cannot be
...
middle of the page footer. But this wouldn't harmonise with the
\Pagestyle{scrheadings}, so \Pagestyle{plain.scrheadings} won't print a page
number. But like \Pagestyle{plain} it won't print a running head.
...
opposite to colour wouldn't make sense and is not recommended for these
elements.%
...
commands. Nevertheless, these commands are deprecated and shouldn't be used
