...
% Translation of version 2011/09/20 of the German file.

\ProvidesFile{common-1.tex}[2012/01/31 KOMA-Script guide (common paragraphs)]
...
\IfCommon{typearea}{Those who achive their documents in source code set utmost
  value to the fact, that future \LaTeX{} runs will result exactly in the same
  ...
  this is not wanted.}%
\IfCommon{maincls}{Those who achive their documents in source code set utmost
  value to the fact, that future \LaTeX{} runs will result exactly in the same
  result. But sometimes improvement and bug corrections of classes will result
  in changes of the behaviour and make-up. But sometimes this is not wanted.}%
\IfCommon{scrextend}{Those who achive their documents in source code set
  utmost value to the fact, that future \LaTeX{} runs will result exactly in
...
\IfCommon{typearea}{\ifshortversion %
    version~2.96a of \KOMAScript, for \Class{scrlttr2} already since
...
\Option{version}. Compatibility to the lowermost supported \KOMAScript{}
...
With \OptionValue{version}{last} you may select the current recent
version. This means to abstain from any compatibility. Option
\Option{version} without any \PName{value} means the same. \ChangedAt{v3.01a}{\Class{scrbook}\and \Class{scrreprt}\and
...
. Because of this setting of the option using \Macro{KOMAoptions} or
\Macro{KOMAoption} has not been intended.%
%
