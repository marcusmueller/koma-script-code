...
\IndexOption{foldmarks~=\PName{Einstellung}} (see
\autoref{sec:scrlttr2.firstpage},
\DescPageRef{scrlttr2.option.foldmarks}). At present there is only one
...
\DescPageRef{scrlttr2.option.addrfield}) and \Option{backaddress} (see
\autoref{sec:scrlttr2.firstpage},
...
\DescPageRef{scrlttr2.option.locfield}) is also taken into account. As is
...
\DescPageRef{scrlttr2.option.version}) the footer is set independently of
...
\DescRef{maincls.cmd.markright}\IndexCmd{markright} or similar commands, you
...
to be used as the argument of, e.\,g., \DescRef{maincls.cmd.markboth} or
\Macro{footnote}.%
%
\EndIndexGroup
%
...
commands\DescRef{scrlttr2.cmd.setkomafont} und \DescRef{scrlttr2.cmd.addtokomafont} (see
...
\DescPageRef{scrlttr2.option.numericaldate}) will be
