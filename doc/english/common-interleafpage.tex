...
Any previously defined \PName{page style} (see
\autoref{sec:\ThisCommonLabelBase.pagestyle} from
\autopageref{sec:\ThisCommonLabelBase.pagestyle} and
\autoref{cha:scrlayer-scrpage} from \autopageref{cha:scrlayer-scrpage}) can be
used. In addition, \OptionValue{cleardoublepage}{current} is also possible.
...
\IfThisCommonLabelBase{maincls}{\iftrue}{\csname iffalse\endcsname}
  \begin{Example}
    ...
    \iffree{, for example, }{}with:
\begin{lstcode}
  \KOMAoptions{cleardoublepage=plain}
\end{lstcode}
    More information about the \IfThisCommonLabelBase{scrextend}{%
      \DescRef{maincls.pagestyle.plain}}{\DescRef{\LabelBase.pagestyle.plain}}
    page style can be found in \IfThisCommonLabelBase{scrextend}{%
...
\begin{lstcode}
  \KOMAoptions{cleardoublepage=plain}
\end{lstcode}
      More information about the \DescRef{maincls.pagestyle.plain} page style
      can be found in \autoref{sec:maincls.pagestyle},
      \DescPageRef{maincls.pagestyle.plain}.
    \end{Example}%
  \fi%
\fi%
...
(see layout option \Option{twoside} in \autoref{sec:typearea.options},
\DescPageRef{typearea.option.twoside}). An empty left page in the current page
