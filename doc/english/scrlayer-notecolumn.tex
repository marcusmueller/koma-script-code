...
would require its own colour management using a so-called \emph{colour stack}.
Because the package was designed only as a \emph{proof of concept}, colour
changes are only generally possible by means of the attributes of the font
element \FontElement{notecolumn.\PName{note column name}} in order to avoid
the time and effort required to implement such colour management.

The second issue is of a more conceptional nature. The content of the auxiliary
file that contains the note-column information is read while processing the
page header. This has an effect in particular if this happens while an
environment like \Environment{verbatim} is active. In this case, the
\Macro{catcode} settings of this environment are active while reading the
auxiliary file. This inevitably leads to errors in processing and output.%
\EndIndexGroup
