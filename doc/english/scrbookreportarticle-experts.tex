...
With\ChangedAt[2014/11]{v3.15}{\Class{scrbook}\and \Class{scrreprt}\and
  \Class{scrartcl}} these command you can either define a new section-like
command \Macro{\PName{Name}} or change a already defines sectioning command
...
\begin{longtable}[l]{llp{\dimexpr\linewidth-6\tabcolsep-14em}}
  ...
  \endlastfoot
    ...
    \PValue{tocindent} & \PName{length} & Total indentation from the left
                                          ...
                                          contents corresponding with this
    ...
    \PValue{tocnumlevel} & \PName{integer} & 
                                          A number, denoting the depth of the
                                          entry in the table of contents
                                          corresponding with this heading, if
...
a command, that was not a section-like command before, will be redefined into a
...
\Macro{RedeclareSectionCommand} can only change an existing command into a
section-like command with given \PName{attributes}. There is no pre-validation
to make sure, that \Macro{\PName{name}} was a section-like command before. It
only has to be an already used command sequence \PName{name}.
...
necessary. Similar for the corresponding output of the counter
...
format of the running head, \Macro{\PName{name}markformat}, die font element
...
section-like commands depending on \Macro{scr@startsection}. This is the
style, that \KOMAScript{} uses for headings from \Macro{section} down to
\Macro{subparagraph}. Headings with the style of \Macro{part} or
\Macro{chapter} are supported not yet. The special headings \Macro{addpart},
\Macro{addchap} and \Macro{addsec} on the other hand depend indirectly on
\Macro{part}, \Macro{chapter}, and \Macro{section}. Therefore they also
...
\end{lstcode}
  ...
  tolerance\,---\,the so called \emph{glue}\,---\,in the declaration of gaps:
\begin{lstcode}
  \RedeclareSectionCommand[%
...
using \Macro{DeclareSectionCommand}. So all other values are just reused
unchanged from that initial definition. By the way, the original definition of
\Macro{paragraph} in \Class{scrartcl} comes up to:
