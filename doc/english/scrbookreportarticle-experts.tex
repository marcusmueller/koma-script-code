...
% TODO: Add new translation of
\begin{Declaration}
\DoHook{headings/preinit/\PName{name}}%
\DoHook{headings/postinit/\PName{name}}%
\DoHook{headings/branch/nostar/\PName{name}}%
\DoHook{headings/begingroup/\PName{name}}%
\DoHook{headings/endgroup/\PName{name}}%
\end{Declaration}
\BeginIndex{}{hooks}%
\emph{Sorry, the translation is still missing.}%
\iffalse
...
Wichtig\important{Achtung!} ist, das keiner dieser Haken für Anweisungen
...
\KOMAScript{} gebotenen Mitteln.

Der\textnote{\DoHook{\dots/preinit/\dots}} Haken
\DoHook{headings/preinit/\PName{name}} wird unmittelbar vor der
...
\DoHook{headings/postinit/\PName{name}} wird etwas später ausgeführt. Zu
diesen Zeitpunkt wurden einige Einstellungen bereits vorgenommen und auch ein
vorausgehender Absatz wurde bereits beendet.

Von\textnote{\DoHook{\dots/branch/\dots}} den Haken
\DoHook{headings/branch/nostar/\PName{name}} oder
\DoHook{headings/branch/star/\PName{name}} wird jeweils nur einer ausgeführt,
...
\DoHook{headings/begingroup/\PName{name}} wird am Anfang der Gruppe
...
\DoHook{headings/endgroup/\PName{name}} vor dem Ende der Gruppe ausgeführt, in
...
% TODO: Fix new translation
Additionally to the commands above, the \KOMAScript{} classes need to redefine
\Macro{@ssect}\IndexCmd{@ssect}\important{\Macro{@ssect}} und
\Macro{@xsect}\IndexCmd{@xsect}\important{\Macro{@xsect}}. Incompatibilities
with packages, that also apply to these, cannot be obviated.%
% :ODOT
\EndIndexGroup
