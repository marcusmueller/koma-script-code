...
% Date of the translated German file: 2015/03/22
...
Command \Macro{raggedchapterentry} currently only provides to print chapter
entries at the table of contents justified or left-aligned. Default is an
empty definition with justified results. For left-aligned the command has to
be redefined to be
...
\autopageref{desc:maincls-experts.cmd.raggedsection} for this. Other
definitions are not recommended!%
%
...
\PValue{1ex} but also \Length{baselineskip} depends on the default font size
...
Since \KOMAScript~3.15\ChangedAt{3.15}{\Class{scrbook}\and \Class{scrreprt}\and
  \Class{scrartcl}} the defaults of these seven commands are independent from
option \Option{headings} (see
\autoref{sec:maincls.structure},
...
\Macro{chapterheadstartvskip}. Correspondingly is sets the length to the value
of attribute \PValue{afterskip} before calling \Macro{chapterheadendvskip}. If
you redefine \Macro{chapterheadstartvskip} or \Macro{chapterheadendvskip} you
should also correspond the new definition to \Length{@tempskipa} to respect
the values of \Macro{DeclareSectionCommand}.

The default of the distances of \Macro{part} do not depend on option
\Option{headings}. So the corresponding commands will not be redefined using
the options. The original definitions of these commands of \Class{scrbook} and
\Class{scrreprt} reads:
\begin{lstcode}
...
\end{lstcode}
Again \Macro{part} sets \Length{@tempskipa} to the values of
\Macro{DeclareSectionCommand}\IndexCmd{DeclareSectionCommand} before the
internal usage of the command.

It is not recommended to redefine the command for the distances above or below
the headings to only change these distances, because you can reconfigure these
distances much easier using \Macro{RedeclareSectionCommand}. Redefinition of
the commands should be reserved to more complex changes. An\textnote{Example!}
...
heading, may be found at \cite{homepage} (in German).%
%
