...
% Date of the translated German file: 2019-07-26
...
The value of the \Option{@else@} option is the unknown option and value of
the \PName{option list}.
% :ODOT
\KOMAScript{} itself uses this feature, for example
...
% TODO: Fix new translation
There\ChangedAt{v3.27}{\Package{scrbase}} is also a handling for an option
called \Option{@else@}\important{\Option{@else@}} similar to
\DescRef{\LabelBase.cmd.FamilyExecuteOptions}. If a family member has such an
option it is used whenever an option of the \PName{option list} is unknown for
the member. If the family itself has an option \Option{@else@} it is used
only, if for an option from the \PName{option list} neither the family nor a
member of the family has executed
...
% TODO: Fix new translation
The\ChangedAt{v3.27}{\Package{scrbase}} handling of an option \Option{@else@}
is equal to \DescRef{\LabelBase.cmd.FamilyOptions}.%
% :ODOT
\EndIndexGroup
...
\begin{Declaration}
  ...
  \Macro{FamilySetInverseBool}\Parameter{family}
                              ...
                              \Parameter{value}
\end{Declaration}
% TODO: Fix the new translation
These\ChangedAt{v3.27}{\Package{scrbase}} two commands differ from
\DescRef{\LabelBase.cmd.FamilyBoolKey} respectively
\DescRef{\LabelBase.cmd.FamilySetBool} only in the inverse boolean logic they
are using. Inverse logic means, the values \PValue{true}, \PValue{yes}, and
...
distinguish between automatic and manual running heads. Usually with automatic
running heads it sets both marks. With manual running heads it does not set
any mark. If someone wants to know whether or not automatic running heads are
active, package authors often compare \DescRef{scrlayer.cmd.@mkboth} with
\DescRef{maincls.cmd.markboth}\IndexCmd{markboth} or \Macro{@gobbletwo}. But
this does not take care for all kind of possible definitions of
\DescRef{scrlayer.cmd.@mkboth}. Because of this, \Macro{IfActiveMkBoth} tests
whether or not \DescRef{scrlayer.cmd.@mkboth} really would set a mark. The
test even accomplishes the usage of \Macro{marks} in the definition of
\DescRef{scrlayer.cmd.@mkboth}. If it recognises such an active
\DescRef{scrlayer.cmd.@mkboth} \PName{then code} is executed. In all other
cases \PName{else code} is used.%
\begin{Example}
  Assuming, in a package you try to set only the right mark if and only if
  automatic running heads, i.\,e. of pagestyle
  \PageStyle{headings}\IndexPagestyle{headings}, are used. A first suggestion
  for this could be:
\begin{lstcode}
  \ifx\@mkboth\markboth \markright{running head}\fi
\end{lstcode}
  Later you recognise, that some package does not use the usual
\begin{lstcode}
  \let\@mkboth\markboth
\end{lstcode}
  but
...
\end{lstcode}
  Unfortunately, now \DescRef{maincls.cmd.markright} is also called in case of
  manual running heads, because someone defined
\begin{lstcode}
...
\end{lstcode}
  ...
  Fortunately both problems can be solved easily using \Package{scrbase}:
\begin{lstcode}
  \IfActiveMKBoth{\markright{Kolumnentitel}}{}
\end{lstcode}%
\end{Example}
By the way, a slightly more simple solution for the problem from the example
...
should be very familiar with \Macro{AtBeginDocument} and \Macro{AtEndDocument}.
\KOMAScript{} offers similar features in some places, for example to hook code
...
combined with a slash after the name to form a new name, and the code with this
name is executed. This process continues until the code for the hook with the
name \PName{specifier} has been executed and the argument is empty.
