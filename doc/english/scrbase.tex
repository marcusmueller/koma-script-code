...
% Date of the translated German file: 2019-11-07
...
\end{Declaration}
\iffalse % Deprecated --> no user documentation needed
...
Therefore, \Macro{FamilyElseValues} is now deprecated. %
\else%
Since version 3.12 the command is deprecated. %
\fi%
Nevertheless,
...
\Macro{PassOptionsToPackage}\IndexCmd{PassOptionsToPackage}. Especially if you
do not use a \KOMAScript{} class the second one is recommended.
\begin{Example}
  The user does not want \Package{scrbase} to define the
  \DescRef{\LabelBase.cmd.ifpdftex} and
  \DescRef{\LabelBase.cmd.ifVTeX} commands. So to load the class,
  the user writes:
\begin{lstcode}
  \documentclass%
...
\end{lstcode}
  ...
  commands \DescRef{\LabelBase.cmd.ifpdftex} and
  \DescRef{\LabelBase.cmd.ifVTeX} are explained later in this
  section.

% TODO: Fix new translation
  If \Class{foo} is not a \KOMAScript{} class in the \File{log} file of the
  \LaTeX{} run may occur a warning message:
\begin{lstoutput}
  LaTeX Warning: Unused global option(s):
       [internalonly=/ifpdftex/ifVTeX].
\end{lstoutput}
  Such a false warning usually occurs if a class loads \Package{scrbase}
  before it processes its own options. That is the usual case. Therefore users
  are recommended to set the option with \Macro{PassOptionsToPackage} before
  loading the class. With
\begin{lstcode}
  \PassOptionsToPackage{internalonly=/ifpdftex/ifVTeX}
                       {scrbase}
  \documentclass{foo}
\end{lstcode}
...
% TODO: Fix missing translation (and remove the \iffalse and \fi)
\iffalse%
