...
\Macro{scr@\PName{name}}, which are internal commands. \KOMAScript{}
...
\Macro{\PName{name}} normally. But eventually, other packages may provide commands
with the same name but different syntax or different functionality. As this would
result in an conflict, \Package{scrbase} can suppress the
definition of the user commands \Macro{\PName{name}}. Using option
...
\PName{family}. If a \PName{family member} isn't given, the member
...
and before the keys of the family members.
\begin{Example}
  Assume option \Option{coldcut} should be set by default already in the
...
value \PValue{true} or value \PValue{false}. There aren't any error message on
...
such options. Thereby the arguments \PName{family member}, \PName{family}, and
\PName{key} are same like used by \Macro{DefineFamilyKey} (see
...
isn't a optional argument for the \PName{default}, \Macro{FamilyStringKey} is
the same like
\begin{quote}
\Macro{DefineFamilyKey}\OParameter{family member}\Parameter{family}%
\Parameter{key}\\
...
expansions are same, the \PName{then instructions} will be executed, otherwise
the \PName{else instructions}. There isn't a corresponding internal command.
%
\EndIndex{Cmd}{ifstr}%
...
Both\ChangedAt{v3.12}{\Package{scrbase}} \PName{string} arguments will be
...
\PName{then instructions} will be executed, otherwise the \PName{else
  instructions}. The command is not completely expandable and there isn't a
...
\end{lstcode}
  ...
  \PValue{UKenglish} you don't need an additional
\begin{lstcode}
...
\Macro{captions\PName{language}} and will do so automatically. The new star
variants of the commands always use \Macro{extras\PName{language}}. So
redefinition of language depending terms of packages like \Package{hyperref}
