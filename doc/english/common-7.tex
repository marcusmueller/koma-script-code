...
\autoref{sec:maincls.oddOrEven}.
\else %------------------------------------------------------- nur maincls -
\BeginIndex{}{page>odd}%
\BeginIndex{}{page>even}%

At double-sided documents we distinguish left and right pages. Left pages
always have an even page number, right pages always have an odd page
number. For this they are mostly called even and odd pages so they are in this
\iffree{guide}{book}. %
\IfCommon{maincls}{This also means, that the detection
  of a left or right page is same like detection of even and odd page
  numbers.

% Umbruchkorrekturtext
  \iftrue%
  There's no distinction in left and right pages at single-side
...
\begin{Example}
  Assuming that you simply want to show, whether a text will be placed onto an
  even or odd page. You may achieve that, i.\,e., using:
