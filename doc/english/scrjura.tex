...
\end{lstcode}
This produces the section mark, \Macro{S}\IndexCmd{S}, followed by a
...
You can find the possible values for \PName{simple switch} in
...
with various settings. You can find the possible \PName{values} and their
meanings in \autoref{tab:scrjura.clausemark}.%
%
\begin{table}
  \caption{Possible values for the \Option{clausemark} option to activate
    running heads}%
...
possible with other \LaTeX{} environments, packages, or classes. The main
reason for this is that \Package{scrjura} is very specialised and far beyond
the author's ordinary practice. So the author mostly relies on detailed user
feedback.%
\EndIndexGroup
