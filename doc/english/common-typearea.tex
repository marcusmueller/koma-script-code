...
\KOMAScript{} delegates this work to \hyperref[cha:typearea]{the package
  \Package{typearea}}\IndexPackage{typearea}. This package can also be used
with other classes. The \KOMAScript{} classes, however, load
\Package{typearea} on their own. Therefore, it's neither necessary nor
sensible to load the package explicitly with \Macro{usepackage} while using a
\KOMAScript{} class. See also \autoref{sec:\ThisCommonLabelBase.options},
...
typesetting, see the documentation for \hyperref[cha:typearea]{the
  \Package{typearea} package}\IndexPackage{typearea}. You can find it in
\autoref{cha:typearea}, starting on \autopageref{cha:typearea}.
