...
{\KOMAScript}\ChangedAt{v3.00}{\Class{scrreprt}\and \Class{scrartcl}}%
offers the option to include or exclude the abstract's title
with the \Option{abstract} option. For \PName{simple switch}, any value
from \autoref{tab:truefalseswitch}, \autopageref{tab:truefalseswitch} can be
used. The default for \KOMAScript{} is \PValue{false}.
...
prospectus. Therefore, the class \Class{scrbook} has no
...
an \Environment{abstract} environment. When using the class \Class{scrreprt},
...
(see \autoref{sec:\LabelBase.lists},
\DescPageRef{\LabelBase.env.quotation}). This way paragraphs will be set with
the first line indented. If the first paragraph of the abstract should not be
indented, this indent can be suppressed by using
\Macro{noindent}\IndexCmd{noindent}%
...
Often you also find additional lists of the floating environments, e.\,g.,
...
at best, an unnumbered entry in the table of contents, and that can be
achieved with
...
You can find an overview of all possible values for the \PName{setting} of
\Option{toc}.  in \autoref{tab:\LabelBase.toc}.

\begin{desclist}
  \desccaption[{Possible values of the \Option{toc} option}]{%
    Possible values of the \Option{toc} option to set the format and contents
    of the table of contents\label{tab:\LabelBase.toc}%
  }{%
    Possible values of the \Option{toc} option (\emph{continued})%
  }%
  \entry{\PValue{bibliography}, \PValue{bib}}{%
    The bibliography has an entry in the table of contents but will not be
    numbered.%
  ...
  \entry{\PValue{bibliographynumbered}, \PValue{bibnumbered},
    ...
    The bibliography will have an entry in the table of contents and will be
    numbered.%
  ...
  \entry{\PValue{flat}, \PValue{left}}{%
    ...
    headings will be in the first column, the heading text in the second
    column, and the page number in the third column. The amount of space
    needed for the numbers of the headings will be determined by the required
    amount of space detected during the previous \LaTeX{} run.%
  ...
  \entry{\PValue{graduated}, \PValue{indent}, \PValue{indented}}{%
    ...
    space for the heading numbers is limited. This corresponds to the default
    setting.%
  ...
  \entry{\PValue{index}, \PValue{idx}}{%
    The index will have an entry in the table of contents but will not be
    numbered.%
    \IndexOption{toc~=\textKValue{index}}}%
  \entry{\PValue{indexnumbered}}{%
    \ChangedAt{v3.18}{\Class{scrbook}\and \Class{scrreprt}\and
      \Class{scrartcl}}%
    The index will have an entry in the table of contents and will be
    numbered.%
    \IndexOption{toc~=\textKValue{index}}}%
  \entry{\PValue{leftaligntextentries}, \PValue{leftalignunnumbered},
    ...
    This will place unnumbered entries left-aligned with the number of
    numbered entries of the same level. This corresponds to the default
    setting.%
    \IndexOption{toc~=\textKValue{numberline}}}%
  \pventry{listof}{%
    The lists of floating environments, e.\,g., figures and tables, will have
    entries in the table of contents but will not be numbered.%
    \IndexOption{toc~=\textKValue{listof}}}%
  \entry{\PValue{listofnumbered}, \PValue{numberedlistof}}{%
    The lists of floating environments, e.\,g., figures and tables, will have
    entries in the table of contents and will be numbered.%
    \IndexOption{toc~=\textKValue{listofnumbered}}}%
  \entry{\PValue{nobibliography}, \PValue{nobib}}{%
    The bibliography will not have an entry in the table of contents. This
    corresponds to the default setting.%
    \IndexOption{toc~=\textKValue{nobibliography}}}%
  \entry{\PValue{noindex}, \PValue{noidx}}{%
    The index will not have an entry in the table of contents. This
    corresponds to the default setting.%
    \IndexOption{toc~=\textKValue{noindex}}}%
  \pventry{nolistof}{%
    The lists of floating environments, e.\,g., figures and tables, will not
...
the table of contents. For the \PName{simple switch}, any value from
...
after every change. The option \DescRef{\LabelBase.option.toc} described above
...
However\ChangedAt{v3.15}{\Class{scrbook}\and \Class{scrreprt}\and
  \Class{scrartcl}}, this behaviour can be changed with the previously
...
The font style of the page numbers can be set separately from these elements
...
option \DescRef{\LabelBase.option.toc}%
\IndexOption{toc~=\textKValue{chapterentrywithdots}}%
\IndexOption{toc~=\textKValue{sectionentrywithdots}}
...
\IndexOption{sectionentrydots~=\PName{simple switch}}, you can change
...
the counter \Counter{tocdepth}. This has the value -1 for
...
sectioning level with the appropriate value which can be used to set
\Counter{tocdepth}.
...
\end{Declaration}%
These\ChangedAt{v3.00}{\Class{scrbook}\and \Class{scrreprt}\and
  \Class{scrartcl}} options can be used to specify whether a horizontal line
will appear beneath the header or above the footer. You can use any of the
values for simple switches shown in \autoref{tab:truefalseswitch},
...
Similarly, activating the \Option{footsepline} option results in a line above
the footer. Deactivating either option will switch off the respective line.
...
\item[{\PageStyle{myheadings}%
  ...
  user. The \DescRef{\LabelBase.cmd.markboth}\IndexCmd{markboth} and
  \DescRef{\LabelBase.cmd.markright}\IndexCmd{markright}\important{%
    \DescRef{\LabelBase.cmd.markboth}\\
    \DescRef{\LabelBase.cmd.markright}} commands can be used for that purpose
  (see \DescPageRef{\LabelBase.cmd.markright}).
...
The\important{\Macro{pagestyle}} page style can be set at any time with the
help of the \Macro{pagestyle} command and takes effect with the next page that
is output. If\textnote{Hint!} you use \Macro{pagestyle} just before a command
that results in an implicit page break and if the new page style should be
used on the resulting new page, a \DescRef{\LabelBase.cmd.cleardoublepage}
just before \Macro{pagestyle} will be useful. But usually you set the page
style only once, at the beginning of the document or in the preamble.
...
Please\textnote{Attention!} note that when you use the
\hyperref[cha:scrlayer-scrpage]{\Package{scrlayer-scrpage}}%
\important{\hyperref[cha:scrlayer-scrpage]{\Package{scrlayer-scrpage}}}
package, switching between automatic and manual running heads is no longer
accomplished by changing the page styles but with special instructions. The
\PValue{headings} and \PValue{myheadings} page styles should not be used with
this package.
...
\end{lstcode}
  As you can see, the previous example uses the element
...
\DescRef{tocbasic.cmd.MakeMarkcase}\IndexCmd{MakeMarkcase}, it is recommended
in such cases that you use the
...
\DescRef{\LabelBase-experts.cmd.thepage}\IndexCmd{thepage} is not used
directly for the page number rather than the
\DescRef{\LabelBase-experts.cmd.pagemark}\IndexCmd{pagemark} which is provided
for it.%
\EndIndexGroup
%
\EndIndexGroup
...
These commands can be used with other page styles too. However, when combined
with automatic running heads, e.\,g., with the
...
the command \DescRef{\LabelBase.cmd.thispagestyle}. Which page style this
...
\DescRef{\LabelBase.pagestyle.plain}\IndexPagestyle{plain}. The meaning of
these macros can be found in \autoref{tab:specialpagestyles}.
%
\begin{table}
  \centering
...
\end{lstcode}
  command. Before you try this, you should read more about the background to
  \DescRef{maincls-experts.cmd.rightmark} in
  \autoref{sec:maincls-experts.addInfos} on
  \DescPageRef{maincls-experts.cmd.rightmark}. The remarks on 
  \DescRef{scrlayer-scrpage-experts.cmd.rightfirstmark} starting on
  page \DescPageRef{scrlayer-scrpage-experts.cmd.rightfirstmark} in
  \autoref{cha:scrlayer-scrpage-experts}, \autoref{part:forExperts} can also
...
\IndexCmd{cleardoubleoddpage}. The possible settings for
\PName{numbering style} can be found in \autoref{tab:numberKind}.
%
\begin{table}
  %  \centering
  \KOMAoptions{captions=topbeside}%
...
the header are not numbered. However, section headings will be numbered
...
matter, this command can be omitted. The default page numbering in the main
...
the command \DescRef{\LabelBase.cmd.pagenumbering}%
\important{\DescRef{\LabelBase.cmd.pagenumbering}} in
...
left-hand page of a two-page spread. This can be accomplished with the
\PName{method} \PValue{left}\ChangedAt{v3.00}{\Class{scrbook}\and
	\Class{scrreprt}}. A summary of the possible values can be found in
...
\begin{table}
  \caption[{Possible values for the \Option{open} option}]{Possible values for
    the \Option{open} option to select page breaks with interleaf pages using
...
option. Any value from \autoref{tab:truefalseswitch},
\autopageref{tab:truefalseswitch} can be used as the \PName{simple
  switch}. The default, however, is \OptionValue{chapterprefix}{false}, the
opposite behaviour\textnote{\KOMAScript{} vs. standard classes} of the
standard classes, which corresponds to
\OptionValue{chapterprefix}{true}. These options also affect the automatic
running heads in the headers (see \autoref{sec:\LabelBase.pagestyle},
\DescPageRef{\LabelBase.pagestyle.headings}).

Sometimes you want to use the simplified chapter headings produced by 
...
\OptionValue{chapterprefix}{true} or \OptionValue{appendixprefix}{true} can be
...
Additional settings for chapter headings can be found in the explanation for 
...
The\OnlyAt{\Class{scrbook}\and \Class{scrreprt}} method to specify the page on
which new chapters begin can be set with 
...
\DescRef{\LabelBase.cmd.subparagraph}. Its function and meaning%
\important{\OptionValue{headings}{optiontohead}\\
  \OptionValue{headings}{optiontotoc}\\
  \OptionValue{headings}{optiontoheadandtoc}} can be altered with the options
...
For a summary of all available settings for the \Option{headings} option, see
\autoref{tab:\LabelBase.headings}. Examples that use some of these settings
are included in the descriptions of the sectioning commands below.
\begin{desclist}
  \desccaption[{Possible values for the \Option{headings} option}]{%
    Possible values for the \Option{headings} option to format
    section headings%
    \label{tab:\LabelBase.headings}%
  }{%
    Possible values for the \Option{headings} option (\emph{continued})%
  }%
  \pventry{big}{%
  ...
  \entry{\PValue{onelinechapter}, \PValue{nochapterprefix},
    ...
    Chapter titles will be set like other headings.%
  }%
  \pventry{openany}{%
  ...
  \pventry{optiontotoc}{%
    ...
    sectioning commands \DescRef{\LabelBase.cmd.part} down to
    \DescRef{\LabelBase.cmd.subparagraph} is activated. By default, the
    optional argument will be used only for the table of contents.%
    \IndexOption{headings~=\textKValue{optiontohead}}%
  }%
  \pventry{small}{%
...
Sometimes other languages have different rules. Therefore it is possible to
force the use of the final point with the option
...
A summary of the possible values for the \PName{setting} of \PName{numbers}
can be found in \autoref{tab:\LabelBase.numbers}. Unlike most other settings,
this option can only be set in the document preamble, i.\,e., before
\Macro{begin}\PParameter{document}.

\begin{table}
  \caption[{Possible values for the \Option{numbers} option}]{Possible values
...
In addition, the \Option{chapteratlists} option can be used to change the
...
is 10\Unit{pt} (see option \DescRef{\LabelBase.option.version} in
\autoref{sec:\LabelBase.compatibilityOptions},
\DescPageRef{\LabelBase.option.version}).
...
Please\textnote{Attention!} note that changes to this option will only take
...
in the standard classes. Thus, an alternative text for the table of contents
and running heads can be specified as an optional argument to the sectioning
commands.
...
\begin{lstcode}
  \section[head={0, 1, 2, 3, \dots}]
          {Natural Numbers Including the Zero}
\end{lstcode}
  will make ``\lstinline|0, 1, 2, 3, \dots|'' the argument of
...
number before the chapter title, but the original behaviour can be restored
...
The\ChangedAt{v2.8p}{\Class{scrbook}\and \Class{scrreprt}\and
  \Class{scrartcl}} font for all headings can be changed with the commands
...
%  \caption
  ...
  \begin{tabular}[t]{lll}
    ...
    \OptionValueRef{\LabelBase}{headings}{big}
      ...
      & \Macro{huge} \\
      & \DescRef{\LabelBase.fontelement.chapterprefix}\IndexFontElement{chapterprefix}
...
The command \Macro{minisec} is designed for this situation. This heading is
...
The font of the sectioning command \Macro{minisec} can be changed using the
...
The formatting commands can be redefined using \Macro{renewcommand} to fit
them to your personal needs. The following default definitions are used by
...
\end{lstcode}
  ...
  the rule. More details concerning class options can be found at
...
\end{lstcode}
  ...
  More about the optional arguments of \Macro{makebox} can be found in
  \cite{latex:usrguide}.
\end{Example}
...
used internally by {\KOMAScript} and also made available to the user. Later it
will be shown how they can be used, for example, to redefine other commands.%
...
\begin{Explain}%
  ...
  Although\OnlyAt{\Class{scrartcl}} there is no command \Macro{chaptermark} or
  \Macro{chaptermarkformat} in \Class{scrartcl}, there are two commands,
  \Macro{subsectionmark} and \Macro{subsectionmarkformat}, which exist only in
  \Class{scrartcl}. However using the
  \hyperref[cha:scrlayer-scrpage]{\Package{scrlayer-scrpage}}%
...
\end{lstcode}
\end{Example}
As you can see, both the commands explained above,
\DescRef{\LabelBase.cmd.chapapp} and
\DescRef{\LabelBase.cmd.chapappifchapterprefix}, are used here.%
\EndIndexGroup
...
packages. Thus, it is recommended never to redefine them.

Do\textnote{Attention!} not confuse the counters \Counter{secnumdepth} and
\DescRef{\LabelBase.counter.tocdepth} (see \autoref{sec:\LabelBase.toc}, 
\DescPageRef{\LabelBase.counter.tocdepth}). Depending on the class you are 
using, the meaning of the values of the counters \Counter{secnumdepth} and 
\DescRef{\LabelBase.counter.tocdepth} may differ for the same section level.%
%
\EndIndexGroup
...
width. In that case, the option for horizontal positioning will have no
effect. You can combine exactly one letter from the vertical with one letter
from the horizontal positioning.

A typical use for \Macro{setchapterpreamble} would be something like an
epigraph, wise saying or dictum. The
\DescRef{\LabelBase.cmd.dictum}\IndexCmd{dictum} command, which can be used
...
heading will be set within the existing vertical space above the heading. The
...
simple mathematics, the \Package{amsmath} package is recommended (see
...
equations are then numbered on the left. This\textnote{Attention!} option must
be used as an optional argument of \DescRef{\LabelBase.cmd.documentclass}.
Usage as an argument of \DescRef{\LabelBase.cmd.KOMAoptions} or
...
\File{fleqn.clo}. Displayed equations are then left-justified. This option can
be used as an optional argument of \DescRef{\LabelBase.cmd.documentclass} but
not as an argument of \DescRef{\LabelBase.cmd.KOMAoptions} or
\DescRef{\LabelBase.cmd.KOMAoption}. The latter would not make sense, because
the recommended maths package \Package{amsmath} can only respond to this
option at load time and does not react to run-time changes of the option.
%
\EndIndexGroup
%
\EndIndexGroup
...
Frequently,\textnote{Attention!} these floating environments%
\Index[indexmain]{floating environments} are not properly understood by
beginners. They often ask to specify the exact position of a table or figure
within the text. However, this is usually unnecessary, since the text will
contain references to these floating environments. It is also not sensible
because such an object can only be set on the page if there is enough space
left for it. If this is not the case, the object would have to be shifted onto
the next page, possibly leaving a huge empty space on the previous page.
...
\important{\Option{captions}{tableheading}} to appear as a heading instead of a
signature. That's because tables can span multiple pages. With such tables,
...
for this environment can also be changed with the \Option{caption} option. The
possible values for the corresponding \PName{settings} can be found in
\autoref{tab:\LabelBase.captions}.
%
\begin{desclist}
  \desccaption[{Possible values for the \Option{captions}} option]{%
    Possible values for the \Option{captions} option for setting formatting of
    captions as headings or signatures in floating environments%
    \label{tab:\LabelBase.captions}%
  }{%
    Possible values for the \Option{captions} option (\emph{continued})%
  }%
  ...
  \entry{\PValue{figureheading}, \PValue{figureabove}, \PValue{abovefigure},
    ...
    \ChangedAt{v3.09}{\Class{scrbook}\and \Class{scrreprt}\and
      \Class{scrartcl}}%
    Captions for figures will use heading format\,---\,possibly deviating from
    \OptionValue{captions}{signature}.%
  }%
  \entry{\PValue{figuresignature}, \PValue{belowfigure},
    ...
    \ChangedAt{v3.09}{\Class{scrbook}\and \Class{scrreprt}\and
      \Class{scrartcl}}%
    Captions for figures will use signature format\,---\,possibly deviating 
    from \OptionValue{captions}{headings}.%
  }%
  \entry{\PValue{heading}, \PValue{above}, \PValue{top}%
    \IndexOption{captions~=\textKValue{heading}}}{%
    \ChangedAt{v3.09}{\Class{scrbook}\and \Class{scrreprt}\and
      \Class{scrartcl}} Captions for floating environments will use formatting
  ...
  \entry{\PValue{leftbeside},
    ...
    Captions for the \DescRef{\LabelBase.env.captionbeside} environment (see
    \autoref{sec:\LabelBase.floats},
    \DescPageRef{\LabelBase.env.captionbeside}) will be placed to the left of
  ...
  \entry{\PValue{outerbeside}, \PValue{besideouter}%
    ...
    Captions for the \DescRef{\LabelBase.env.captionbeside} environment (see
    \autoref{sec:\LabelBase.floats},
    \DescPageRef{\LabelBase.env.captionbeside}) will be placed outside of and
  ...
  \entry{\PValue{signature}, \PValue{below}, \PValue{bot}, \PValue{bottom}%
    ...
    \ChangedAt{v3.09}{\Class{scrbook}\and \Class{scrreprt}\and
      \Class{scrartcl}}%
    Captions for floating environments will use signature format. This setting 
  ...
  \entry{\PValue{tableheading}, \PValue{tableabove}, \PValue{abovetable},
    ...
    Captions for tables will use heading format\,---\,possibly deviating from
    \OptionValue{captions}{signature}.%
  }%
  \entry{\PValue{tablesignature}, \PValue{belowtable}, \PValue{belowtabular},
    ...
    Captions for tables will use signature format\,---\,possibly deviating
...
For tables and figures, or in general for all floating environments,
the behaviour of \Macro{caption} can be controlled with the 
...
\Macro{captionbelow}. However, it
is recommended\textnote{Hint!} to use the 
...
\Macro{captionabove}. Alternatively, you can use
\Macro{captionabove} instead of \Macro{caption} inside every
...
\end{lstcode}
  ...
  of course use the option \OptionValueRef{\LabelBase}{captions}{tableheading}
...
command \Macro{captionof}, with which you can put a caption for a floating
...
\end{figure}%
\end{Example}%
A non-floating table with a caption can be produced in the same way as the
...
\KOMAScript{} offers a special environment for this which can be used within
...
Whether\important{\PName{placement}} the \PName{caption text} is placed on the
left or the right can be determined by the parameter \PName{placement}.
Exactly one of the following letters is allowed:
...
\end{labeling}
The default placement is to the right of the content of the environment. This
default\ChangedAt{v3.00}{\Class{scrbook}\and \Class{scrreprt}\and
  \Class{scrartcl}} can be changed using the
...
usually the bottom of the figure. This can be changed, e.\,g., using
\Macro{raisebox}\IndexCmd{raisebox}.

\begin{Example}
  An example using the \Environment{captionbeside} environment can be found in
...
\fi
  ...
  In contrast, the top-aligned format seen in
  \autoref{fig:\LabelBase.captionbesidetop} can be used. To illustrate how to
  shift the baseline using \Macro{raisebox}, here is a complete example. Such
  a shift can be applied not only to a substitute figure, as previously
  shown, but also, for example, to \Macro{includegraphics} (see
...
redefining the commands for the output of the counters \Macro{thefigure} or
\Macro{thetable}. Such a redefinition would have unwanted side effects on the
output of \Macro{ref}, the table of contents, the list of figures, and the
list of tables. Instead, {\KOMAScript} offers two \Macro{\dots format}
commands. These have the following defaults:
\begin{lstcode}
...
be changed at any time with the \Macro{setcapindent} or \Macro{setcapindent*}
command. Here the parameter \PName{indent} determines the indentation of the
second and subsequent lines. If you want a line break after the label and
before the caption text, then you can define the indentation \PName{xindent}
of the caption text with the starred version of the command instead:
\Macro{setcapindent*}.
...
possible at any time with \Macro{setcaptionalignment}. Exactly one of the
letters listed in \autoref{tab:maincls.captionalignment} can be specified for
the \PName{alignment}. If you specify an unknown \PName{alignment}, you will
...
package are only available if the package was loaded before you use
...
\begin{lstcode}
  \setcaptionalignment{c}
\end{lstcode}
  command to the document preamble. Thereafter, though, you do not like this
  change for table captions at all. Therefore, you use\textnote{only for
    figures}
\begin{lstcode}
...
\end{lstcode}
  ...
  length. This is caused by the lack of hyphenation, causing long words to wrap
  completely onto the next line, leaving large gaps. You want to allow
...
option has to be used in the optional argument of
...
neither numbered nor included in the table of contents. More information about
this behaviour can be found in \autoref{sec:\LabelBase.toc}. As an alternative
to the settings \OptionValueRef{\LabelBase}{toc}{nolistof}%
\IndexOption{toc~=\textKValue{nolistof}},
\OptionValueRef{\LabelBase}{toc}{listof}\IndexOption{toc~=\textKValue{listof}},
and \OptionValueRef{\LabelBase}{toc}{listofnumbered}%
\IndexOption{toc~=\textKValue{listofnumbered}} mentioned there,
you can also look at this behaviour from perspective of the lists themselves.
Therefore you can achieve the same results with the settings
...
\begin{lstcode}
  ...
  \listoftables
\end{lstcode}
  More information about the command \DescRef{\LabelBase.cmd.addchap*} can be
  found in \autoref{sec:\LabelBase.structure} on
...
than one \LaTeX{} run before the lists of floating environments will reach
their final state.

The \OptionValue{listof}{entryprefix}\ChangedAt{v3.06}{\Class{scrbook}\and
  \Class{scrreprt}\and \Class{scrartcl}} setting will automatically activate
...
However, such prefixes can be added using the
...
A method where only chapters that contain floating environments appear in the
respective lists can be found at \cite{https://komascript.de/comment/5070}. A
more direct control over what appears in the lists of floating environments
can be achieved with the \DescRef{\LabelBase.option.chapteratlists} option,
which is explained in \autoref{sec:\LabelBase.structure}, on
\DescPageRef{\LabelBase.option.chapteratlists}.

An overview of all settings for the \Option{listof} option can be found at
\autoref{tab:\LabelBase.listof}.

\begin{desclist}
  \desccaption[{Possible values for the \Option{listof} option}]{%
    Possible values for the \Option{listof} option for modifying the format
    and contents of the lists of floating
    environments\label{tab:\LabelBase.listof}%
  }{%
    Possible values for the \Option{listof} option (\emph{continued})%
  }%
...
relevant bibliography package! General information about bibliographies can be
found in \cite{lshort}.
...
In addition to the formatting style, one more feature can be selected using
...
A summary of possible values for the \Option{bibliography} option can be
found in \autoref{tab:\LabelBase.bibliography}. Note, however, that new values
can be defined with
\DescRef{maincls-experts.cmd.newbibstyle}\IndexCmd{newbibstyle}.

\begin{table}
  \caption[{Possible values for the \Option{bibliography} option}]{%
...
\begin{Example}
  You want to set the bibliography not justified but ragged right. This can be
  achieved using:
\begin{lstcode}
...
cooperation with packages modifying the bibliography, if you use such a package
(see \autoref{sec:maincls-experts.coexistence},
...
EA summary of the possible values for the \PName{setting} of
\Option{index} can be found in \autoref{tab:maincls.index}.

\begin{table}
  \caption{Possible values for the \Option{index} option}
