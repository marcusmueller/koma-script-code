...
literature, e.\,g. \cite{lshort} and \cite{latex:usrguide}, or the examples
given here. Since the \Package{typearea} package is loaded automatically when
using the main {\KOMAScript} classes, the package options can be given as
class options (see \autoref{sec:maincls.options}).
...
\begin{Example}
  Suppose you create a financial report. The whole thing should be printed
  out one-sided on A4 paper and then stapled in a binder folder.
  The clip of the folder covers 7.5\Unit{mm}. The stack of pages is very
  thin, thus from bending and the folios at most another 0.75\Unit{mm} will be
  lost. Therefore, you can write:
\begin{lstcode}
...
\end{lstcode}
  The option \Option{a4paper} can be omitted with \Class{scrreprt},
  since this is the default for all {\KOMAScript} classes.
...
\DescRef{\LabelBase.cmd.usepackage}, nor you specify it as an optional
...
\begin{table}
  \caption[{%
    ...
    Possible symbolic values for the \DescRef{typearea.option.DIV} option or
...
\begin{table}
  \caption[{%
    ...
    Possible symbolic \PName{BCOR} arguments for
    \Macro{typearea}\OParameter{BCOR}\Parameter{DIV}%
  }
...
results in two-sided printing with one-sided margins and one-sided, i.\,e.,
...
The option \Option{headlines}\important{\Option{headlines}} is set to the
number of header lines. The \Package{typearea} package uses a default of 1.25.
This is a compromise: large enough for underlined headers (see
\autoref{sec:maincls.pagestyle}) and small enough that the relative weight of
the top margin is not affected too much when the header is not underlined.
Thus the default value will usually be adequate. In special cases, however,
you may need to adjust the header height more precisely to your actual
requirements.
...
core {\LaTeX} does not provide a length for the height of the footer. So
...
current binding correction, e.\,g. the value set by option
