...
% Date of the translated German file: 2020-09-11
...
that was requested. \ChangedAt{v3.32}{\Package{scrlfile}}However, a contact
with the \LaTeX team revealed that they do not consider such a procedure to be
recommendable in most cases. Therefore all instructions in this section are
considered deprecated and should not be used anymore. When using \LaTeX{} from
version 2020-10-01 on they are partly not offered at all or generate an error
message.
...
\end{Declaration}%
This\ChangedAt{v3.32}{\Package{scrlfile}} statement applies when using
\LaTeX{} from version 2020-10-01 as deprecated, but currently works
still. It\ChangedAt{v2.96}{\Package{scrlfile}} defines a replacement
...
\end{Declaration}%
These\ChangedAt{v3.32}{\Package{scrlfile}} instructions are considered
deprecated when using \LaTeX{} from version 2020-10-01 on and are not provided
via option \DescRef{\LabelBase.option.withdeprecated} due to a problem. If you
are using an older \LaTeX{} versions, the instructions are also considered
deprecated and will probably be available only for a limited time.
...
Since\ChangedAt{v3.32}{\Package{scrlfile}} the statements \DescRef{\LabelBase.cmd.ReplaceInput}, \DescRef{\LabelBase.cmd.ReplaceClass}, and \DescRef{\LabelBase.cmd.ReplacePackage} are deprecated, the same applies to the commands for cancelling the replacement. They should therefore also no longer be used.% 
\EndIndexGroup
\end{Explain}
