...
\end{Example}
By the way, you can delete the content of the variable by providing
an empty \PName{content} argument. Of course, the \PName{description} of
the variable can be deleted in the same way, with an empty argument for 
the description.
\begin{Example}
...
(see option \DescRef{\LabelBase.option.version} in
...
Note that additional settings can be changed between the individual letters.
...
For example, a cover letter and a CV can be combined in one document.
...
\Package{scrletter} package provide two additional hooks. The \PName{code} for
these can be declared using \Macro{AtBeginLetter} and
...
begins, and a vertical distance\,---\,usually below the signature\,---\,is
inserted. The \Macro{ps} text can be followed by any text. If you want the
postscript to be introduced with the acronym ``PS:'' , which is written
without a full stop, you have to type this yourself. This abbreviation is
printed neither automatically nor optionally by the class \Class{scrlttr2}.
...
\begin{Explain}
  ...
  remain popular. It can be useful to emphasize important points once more, or
  even to mention the less important matters.
\end{Explain}
%
\EndIndexGroup
...
the respective options. Possible values for \Option{fromalign} are shown in
...
\OptionValue{fromrule}{afteraddress}. A summary of all possible settings for
...
\begin{table}
  \caption[{Possible values of the \Option{fromrule} option with
    \Class{scrlttr2}}]{Possible values of the \Option{fromrule} option for the
...
\begin{Example}
  ...
  sender's name. More information about how to configure the signature can be
  found in \autoref{sec:\LabelBase.closing} starting on
...
information. Any standard value for simple switches from
\autoref{tab:truefalseswitch}, \autopageref{tab:truefalseswitch} can be
assigned to these options. The default for all of them is \PValue{false}. The
\PName{contents} themselves are determined by the variables of the same name.
You can find the defaults for the \PName{description} or title of each
variable in \autoref{tab:\LabelBase.fromTerm}. You can find the separators
that will be inserted between the \PName{description} and the \PName{content}
in \autoref{tab:\LabelBase.fromSeparator}.
...
that you have to load package \Package{marvosym} on your own if you activate
...
optional delivery type\Index{delivery type} between the
return address and the recipient address. This will be output only if the
\Variable{specialmail} variable has non-empty content and
\OptionValue{priority}{manual}\ChangedAt{v3.03}{\Class{scrlttr2}} has been
selected, which is the default. Class \Class{scrlttr2} itself does not use the
\PName{description} of the \Variable{specialmail} variable. The alignment is
defined by the \PLength{specialmailindent} and
...
%  \caption
  \begin{captionbeside}
    [{Possible values of option \Option{locfield} with
      \Class{scrlttr2}}]{Possible values of the \Option{locfield} option to
...
additional data. Possible values for this option are shown in
\autoref{tab:\LabelBase.refline}. The default is \PValue{narrow} and
\PValue{dateright}\ChangedAt{v3.09}{\Class{scrlttr2}}.%
%
\begin{table}
  \caption[{Possible values of the \Option{refline} option with
    \Class{scrlttr2}}]{Possible value of the \Option{refline} option to set
...
\FontElement{lettertitle}\important{\FontElement{lettertitle}} element 
with the \DescRef{\LabelBase.cmd.setkomafont} and \DescRef{\LabelBase.cmd.addtokomafont} commands (see
\autoref{sec:\LabelBase.textmarkup},
\DescPageRef{\LabelBase.cmd.setkomafont}). Font size declarations are
allowed. The \Macro{LARGE} font size always precedes the font selection in \KOMAScript{}, and is therefore not part of the default setting
\Macro{normalcolor}\Macro{sffamily}\Macro{bfseries}. With \Class{scrlttr2}, you can also
use \FontElement{title}\important{\FontElement{title}} as an alias for
\FontElement{lettertitle}. This is not possible when using \Package{scrletter} with
a \KOMAScript{} class because these classes already define a 
\FontElement{title} element for the document title with different setting.%
...
possible values are listed in \autoref{tab:\LabelBase.subject}.
...
%  \caption
  \begin{captionbeside}
    [{possible values of the \Option{subject} option with \Class{scrlttr2}}]
    {possible values of the \Option{subject} option for the position and
...
has an effect when the option \DescRef{\LabelBase.option.enlargefirstpage}
...
\end{Declaration}%
\BeginIndex{}{letterhead page>footer}%
The footer of the first page empty by default.
...
positions simultaneously. Possible values are listed in
\autoref{tab:\LabelBase.pagenumber}. The default is \PValue{botcenter}.

\begin{table}
  \caption[{possible values for the \Option{pagenumber} option with
    \Class{scrlttr2}}]{possible values of the \Option{pagenumber} option to
...
\FontElement{pagefoot}\important{\FontElement{pagefoot}} element is
...
obtain flush-right or flush-left alignment inside the box, the command can be
redefined in the same way as \DescRef{maincls.cmd.raggedsection} (see the
...
\end{Example}
More about the \Package{adrconv}\IndexPackage{adrconv} package can be found in
