...
\ProvidesFile{scrlttr2.tex}[2007/08/22 KOMA-Script guide (chapter: scrlttr2)]
...
to the reference line. Normally, the reference line contains at least
the date, but it can hold additional data. Possible values for this
...
\begin{Explain}
  ...
  also contact the {\KOMAScript} author.
\end{Explain}
...
reference fields (see \autoref{sec:scrlttr2.refLine}). The
\PName{description} and the content of the variable are added at the
end of the reference fields. The starred version \Macro{newkomavar*}
is similar to the unstarred version, with a subsequent call of the
command \Macro{addtoreffields}. Thus, the starred version
automatically adds the variable to the reference fields.
\begin{Example}
...
\end{Example}
When you define a variable for the reference fields you should always
...
the class. The reference line is then empty except for the date which
is always appended to the end. This can be useful, for example, if you
wish to change the order of the variables in the reference line.

The command \Macro{defaultreffields} acts to reset the reference line
to its predefined format. In doing so, all custom-defined variables
are removed from the reference line.
%
...
fields. Using the command \Macro{usekomavar} you have access to the
