...
optional argument and \PName{text} as a mandatory argument. An example of such
a command provided by the \KOMAScript{} classes can be found in
...
mark will be an argument of \Macro{MakeMarkcase}. This command can be used to
...
the experimental package \Package{tocstyle} that never became an official part
...
The \PName{option list} can be used to configure the various, usually
...
the entry's page number. The \PName{leader}, which is repeated at regular
intervals, can be specified as an optional argument. The default is a dot.
...
or without an entry in the table of contents, with or without numbering). But with a
simple
\begin{lstcode}
...
entries into the table of contents. A lower sectioning level can be used for
...
\begin{desclist}
  ...
  \entry{\OptionVName{listname}{title}}{%
    Each content list has a heading, or title, that can be specified with this
  ...
  \entry{\OptionVName{tocentrystyle}{TOC-entry style}}{%
    ...
    numerical level of the entries using option \Option{tocentrylevel}. For
    more information about the styles and their attributes see
