...
, \DescPageRef{tocbasic.cmd.tocbasic@extend@babel} for more information.%
\EndIndexGroup
...
\DescPageRef{tocbasic.cmd.tocbasic@starttoc} for more information about
...
\end{lstcode}
  ...
  at \DescPageRef{tocbasic.cmd.setuptoc}
...
\autopageref{example:scrwfile.BeforeStartingTOC}.
\EndIndexGroup
...
\item[\texttt{leveldown}] uses not the top level heading below
  ...
  \Macro{deftocheading}, that user is responsible for the evaluation of the
...
\item[\texttt{onecolumn}] \leavevmode\ChangedAt{v3.01}{\Package{tocbasic}}%
  ...
  \DescPageRef{tocbasic.cmd.AtAddToTocList}) for all TOCs with owner
...
\item[\texttt{chapteratlist}] activates special code to be put into the TOC at
  ...
  \autoref{sec:maincls.floats}, \DescPageRef{maincls.option.listof} for
...
\item[\PValue{gobble}] is the most ordinary style. Independently from the
  setting of
  \DescRef{\LabelBase.counter.tocdepth}\IndexCounter{tocdepth}\important{\DescRef{\LabelBase.counter.tocdepth}},
...
\DescPageRef{tocbasic.cmd.DeclareNewTOC}) you have to prefix the names of
...
\DescPageRef{maincls-experts.cmd.DeclareSectionCommand}) and similar
...
\begin{desclist}
  ...
  \entry{\OptionVName{level}{integer}}{%
    ...
    \DescRef{\LabelBase.counter.tocdepth}\important{\DescRef{\LabelBase.counter.tocdepth}}\IndexCounter{tocdepth}.
  ...
  \entry{\OptionVName{linefill}{code}}{%
    ...
    \DescPageRef{tocbasic.cmd.TOCLineLeaderFill}).
  ...
  \entry{\OptionVName{onstarthigherlevel}{code}}{%
    ...
    \DescPageRef{tocbasic.cmd.LastTOCLevelWasLower}).%
  }%
  \entry{\OptionVName{onstartlowerlevel}{code}}{%
    ...
    \DescPageRef{tocbasic.cmd.LastTOCLevelWasHigher}).%
  }%
  \entry{\OptionVName{onstartsamelevel}{code}}{%
    ...
    \DescPageRef{tocbasic.cmd.LastTOCLevelWasSame}).%
  }%
...
\DescPageRef{tocbasic.cmd.addxcontentsline}). In both cases \LaTeX{}
...
\autoref{sec:tocbasic.toc}, \DescPageRef{tocbasic.cmd.setuptoc}). For the
...
\DescPageRef{tocbasic.cmd.listoftoc*}). Authors of classes or packages
...
\autoref{sec:tocbasic.toc}, \DescPageRef{tocbasic.cmd.listoftoc})
depending on feature \PValue{numberline} (see \autoref{sec:tocbasic.toc},
\DescPageRef{tocbasic.cmd.setuptoc}). This guarantees that changes of the
...
\DescPageRef{tocbasic.cmd.DeclareTOCStyleEntry} for more information
about the styles.%
%
\begin{desclist}
