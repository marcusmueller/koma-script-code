...
% Date of the translated German file: 2021-10-12
...
\begin{desclist}
  ...
  \entry{\OptionVName{counterwithin}{\LaTeX{} counter}}{%
    ...
    However,\ChangedAt{v3.35}{\Package{tocbasic}} the dependency is ignored
    outside the main matter of a document (see
    \DescRef{maincls.cmd.frontmatter}, \DescRef{maincls.cmd.mainmatter} and
    \DescRef{maincls.cmd.backmatter} in \autoref{sec:maincls.separation},
    \DescPageRef{maincls.cmd.frontmatter}). If \Macro{chapter} is defined and
    \OptionValue{counterwithin}{chapter} or otherwise
    \OptionValue{counterwithout}{section} then the dependency is also ignored
    if \PName{\LaTeX{} counter} is less than one on output.%
  }%
  \entry{\Option{float}}{%
