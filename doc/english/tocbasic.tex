...
% Date of the translated German file: 2019-10-07
...
of additional features, that correspond to the title. This is especially true
...
% TODO: Fix new translation
  brings all content-list entry styles provided by \KOMAScript{} to ignore
  attribute \PValue{indent} (see \autoref{tab:tocbasic.tocstyle.attributes},
  \autopageref{tab:tocbasic.tocstyle.attributes.indent}). This deactivates the
  indent of the entry resp. paragraph.%
% :ODOT
...
% TODO: Fix new translation
  is somehow special. All other styles produce one paragraph per entry. But
  this one produces one paragraph for all consecutive entries of this
  ...
  attributes in addition to the default \PValue{level} attribute it
  is almost as high configurable as \PValue{tocline}. This style hardly needs,
  that a still open paragraph is finished at the very beginning of the entry
  of another style and at the end of the current table of contents or list of
  something. So it should never be combined with entries or tables of contents
  or lists of something, that are generated without using mechanisms of
  \Package{tocbasic}.
% :ODOT
...
% TODO: Fix new translation
Beside\ChangedAt{v3.27}{\Package{tocbasic}} the usual assignment with
\Option{\PName{key}=\PName{value}} both commands provide for all styles of the
options of all styles of \KOMAScript{} a copy assignment with
\Option{\PName{key}=\PName{entry level}}. In this case the currently valid
setting of \PName{key} of the \PName{entry level} will be copied. For example,
you can copy the current indent of the \PValue{figure} level using
\OptionValue{indent:}{figure}. For options that expect a length or integer
value you can also use \Option{\PName{key}+=\PName{value}} to add
\PName{value} to the current setting of the \PName{key}. To substract simply
add a negative \PName{value}. For example, with \OptionValue{indent+}{1cm} you
could increase the indent by 1\Unit{cm}.
% :ODOT
...
\Option{tocentrylevel}.
% TODO: Fix new translation
The previously explained copy operation using
\Option{:=} is also provided. But the summation using \Option{+=} is not
provided.
% :ODOT
...
% TODO: Fix new translation
These command neither provide the copy operation with \Option{:=} nor the
...
% TODO: Fix new translation
    The \PName{code} will be executed at the end of the paragraph in which an
    entry with style \PValue{toctext} is printed. If several entries of the
    paragraph has such settings, the code of them is be executed in the
    same order as the entries are printed.%
% :ODOT
  }%
...
% TODO: Fix new translation
    For\ChangedAt{v3.27}{\Package{tocbasic}} style \PValue{toctext} the
    \PName{length} is the horizontal distance of the paragraph with the
    entries. If different entries have different setting, the last one
    wins. For the other style \PName{length} is the horizontal distance of the
    entry from the left margin (see \autoref{fig:tocbasic.dottedtocline} and
...
% TODO: Fix new translation
    If attribute \PValue{noindent} is set for a content list via
...
% TODO: Fix new translation
    The\ChangedAt{v3.27}{\Package{tocbasic}} style \PValue{toctext} always
    adds a horizontal space of width \PName{length} after the number of the
    entry.
% :ODOT
  ...
  \entry{\OptionVName{onendentry}{code}}{%
    \ChangedAt{v3.27}{\Package{tocbasic}}%
% TODO: Fix new translation
    The \PName{code} is executed immediately after an entry with style
    \PValue{toctext}, if this entry is not the last one of the paragraph. The
    user has to take care to not finish the paragraph within \PName{Code}.
...
% :ODOT
  ...
  \entry{\OptionVName{onendlastentry}{code}}{%
    \ChangedAt{v3.27}{\Package{tocbasic}}%
% TODO: Fix new translation
    The \PName{code} is executed immediately before the paragraph of an entry
    with style \PValue{toctext} ends, if this entry is the last one of the
    paragraph. The user should take care to not finish the paragraph within
    \PName{code}.%
% :ODOT
  }%
  \entry{\OptionVName{onstartentry}{code}}{%
    \ChangedAt{v3.27}{\Package{tocbasic}}%
% TODO: Fix new translation
    The \PName{code} is executed immediately before an entry with style
    \PValue{toctext}, if this entry is not the first one of the paragraph.
    The user has to take care to not finish the paragraph within
    \PName{code}.%
% :ODOT
  }%
...
% TODO: Fix new translation
    The \PName{code} is executed immediately before an entry with style
    \PValue{toctext}, if this entry is the first one of the paragraph.
    The user should take care to not already start the paragraph within
    \PName{code}.%
% :ODOT
  }%
  ...
  \entry{\OptionVName{pagenumberwidth}{length}}{%
    \ChangedAt{v3.27}{\Package{tocbasic}}%
% TODO: Fix new translation
    You can use this attribute to locally change the width of the default page
    number box of an entry with the style \PValue{tocline} from
    \Macro{@pnumwidth} to the given \PName{length}. Note, if you change the
    default page number box with attribute \Option{pagenumberbox} this is not
    longer used automatically.%
% :ODOT
  }%
  \entry{\OptionVName{prepagenumber}{code}}{%
% TODO: Fix new translation
    Style \PValue{toctext} executes \PName{code} between the text of an entry
    and the page number of the entry. Usually this is used to add a horizontal
    space or separator between text and page number.%

    The default is to a non breakable space using \Macro{nonbreakspace}.%
% :ODOT
  }%
...
% TODO: Fix new translation
    The \PName{switch} is one of the values for simple switches from
    \autoref{tab:truefalseswitch}, \autopageref{tab:truefalseswitch}. If the
    switch is active for any entry with style \PValue{toctext} inside the same
    paragraph, the whole paragraph is printed ragged-right.
% :ODOT
  }%
  \entry{\OptionVName{rightindent}{length}}{%
    \ChangedAt{v3.27}{\Package{tocbasic}}%
% TODO: Fix new translation
    You can use this attribute to locally change the right indent of the text
    of an entry with style \PValue{tocline} from \Macro{@tocrmarg} to the
    given \PName{length}.%
  }%
% :ODOT
\end{desclist}
...
% TODO: Fix new translation
Beside\ChangedAt{v3.27}{\Package{tocbasic}} the usual options of the form
\Option{\PName{key}=\PName{value}} the five \Macro{DefineTOCEntry\dots Option}
commands automatically define options of the form
\Option{\PName{key}:=\PName{entry level}}. These copy the value of another
\PName{entry level} if the value is stored in a macro with the same
\PName{prefix} and \PName{postfix}. For the styles of \Package{tocbasic} this
is the case for all options of the same name independent from the name of the
style. The commands \Macro{DefineTOCEntryLengthOption} and
\Macro{DefineTOCEntryNumberOption} additionally define options of the form
\Option{\PName{key}:=\PName{value}}, that are used to add the new
\PName{value} to the value already stored in
...
\begin{desclist}
  ...
  \entry{\OptionVName{category}{string}}{%
    \ChangedAt{v3.27}{\Package{tocbasic}}%
% TODO: Fix new translation
    This option can be used as a synonym of \OptionVName{owner}{string}.%
% :ODOT
  }%
