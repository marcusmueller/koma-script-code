...
(\PValue{american}\ChangedAt{v3.13}{\Package{scrdate}}, \PValue{australian},
\PValue{british}, \PValue{canadian}, \PValue{english}, \PValue{UKenglish}, and
USenglish), German (\PValue{austrian}\ChangedAt{v3.08b}{\Package{scrdate}},
\PValue{german}, \PValue{naustrian}, \PValue{ngerman}, \PValue{nswissgerman},
and \PValue{swissgerman}\ChangedAt{v3.13}{\Package{scrdate}}), French
(\PValue{acadian}, \PValue{canadien},
\PValue{francais}\ChangedAt{v3.13}{\Package{scrdate}}, and \PValue{french}),
Italian (\PValue{italian}), Spanish (\PValue{spanish}), Croatian
(\PValue{croatian}), Finnish (\PValue{finnish}), Norwegian (\PValue{norsk}),
Swedish (\PValue{swedish}), Danish (\PValue{danish}), Polish
(\PValue{polish}\ChangedAt{v3.13}{\Package{scrdate}}), and Czech
(\PValue{czech}\ChangedAt{v3.13}{\Package{scrdate}}). If you want to configure
it for other languages, you may use this command. The first argument is the
name of the language and the other arguments are the names of the
corresponding days.
