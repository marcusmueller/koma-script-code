...
\ProvidesFile{common-8.tex}[2012/02/07 KOMA-Script guide (common paragraphs)]
...
\IfNotCommon{scrlttr2}{%
  ...
  Interleaf pages may be found in books mostly. While chapters in books
  commonly start on odd pages, sometimes a left page without contents has to
  be added before. This is also the reason because of that interleaf pages
  only exist at double-side printing. The unused back sides at the one-side
  printings aren't interleaf pages really. Nevertheless at the printing paper
  they seam to be such pages.}
\fi %**************************************************** Ende nur maincls *
\ifIgnoreThis %+++++++++++++++++++++++++++++++++++++++++++++ nicht maincls +
...
\IfCommon{scrlttr2}{%
  ...
  that real two-sided letters are seldom, because binding of letters won't be
...
\autoref{cha:scrpage} from \autopageref{cha:scrpage}) may be used. Beside this
...
\IfNotCommon{scrlttr2}{%
  ...
  page. This is in opposite to the classic typography, nevertheless it may be
  suitable, i.\,e., if the double-page spread of the chapter start is of
  special contents. \KOMAScript{} therefor provides the commands
  \Macro{cleardoubleevenstandardpage}, \Macro{cleardoubleevenplainpage},
  \Macro{cleardoubleevenemptypage}, and \Macro{cleardoubleevenusingstyle}
  equivalent to the odd-page commands.}
...
and is same like one of the corresponding commands above. Same is valid for
the standard command \Macro{cleardoublepage}, that may be either
\Macro{cleardoubleoddpage} or \Macro{cleardoubleevenpage}.

\IfCommon{scrlttr2}{%
  At \Class{scrlttr2} the other commands are there only for completeness. More
  information about may be found at
  \autoref{sec:maincls.emptypage},
