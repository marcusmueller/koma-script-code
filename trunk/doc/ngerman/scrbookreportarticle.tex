...
genauso verfahren sollte. Siehe hierzu in \autoref{sec:\LabelBase.structure} die
...
\DescPageRef{\LabelBase.option.titlepage}) verwendet, so wird die Zusammenfassung
intern mit Hilfe einer \Environment{quotation}-Umgebung\IndexEnv{quotation}
(siehe \autoref{sec:\LabelBase.lists}, \DescPageRef{\LabelBase.env.quotation})
...
geändert werden (siehe \DescPageRef{\LabelBase.option.toc} und
...
\item[{\PageStyle{headings}%
  ...
  \DescPageRef{\LabelBase.option.headsepline}) Linien unter dem Kopf und
...
\item[{\PageStyle{myheadings}%
  ...
  dazu die Anweisungen \DescRef{\LabelBase.cmd.markboth}\IndexCmd{markboth} und
  \DescRef{\LabelBase.cmd.markright}\IndexCmd{markright}\important[i]{%
    \DescRef{\LabelBase.cmd.markboth}\\
    \DescRef{\LabelBase.cmd.markright}} (siehe \DescPageRef{\LabelBase.cmd.markright}).
...
(siehe \autoref{sec:\LabelBase.textmarkup},
\DescPageRef{\LabelBase.cmd.setkomafont}). Für den Kopf und den Fuß ist
...
\DescRef{\LabelBase.option.version}, \autoref{sec:\LabelBase.compatibilityOptions},
...
\Macro{section*} (siehe \autoref{sec:\LabelBase.structure},
\DescPageRef{\LabelBase.cmd.section*} und
...
\Macro{cleardoubleemptypage} (siehe \autoref{sec:\LabelBase.emptypage},
...
auch bei der Option \Option{listof} in \autoref{sec:\LabelBase.floats},
...
\DescRef{\LabelBase.cmd.addtokomafont} (siehe \autoref{sec:\LabelBase.textmarkup},
\DescPageRef{\LabelBase.cmd.setkomafont}) bestimmt werden. Dabei wird
...
(siehe \DescPageRef{\LabelBase.option.headings}). Sie finden die
...
dieses Abschnitts (siehe \DescPageRef{\LabelBase.option.numbers}) erläutert
...
\begin{desclist}
  ...
  \entry{\PValue{innerbeside},
    ...
    Gleitumgebungstitel der Umgebung \Environment{captionsbeside} (siehe
    \autoref{sec:\LabelBase.floats},
    \DescPageRef{\LabelBase.env.captionbeside}) werden bei doppelseitigem
...
oder in das Inhaltsverzeichnis aufgenommen. In \autoref{sec:\LabelBase.toc} wurde
...
\begin{lstcode}
  ...
  \listoftables
\end{lstcode}
  Näheres zur Anweisung \Macro{addchap*} ist \autoref{sec:\LabelBase.structure},
  \DescPageRef{\LabelBase.cmd.addchap*} zu entnehmen.
\end{Example}
...
zu erreichen, die in \autoref{sec:\LabelBase.structure} auf
...
\DescRef{\LabelBase.option.toc}\important{\DescRef{\LabelBase.option.toc}} (siehe \autoref{sec:\LabelBase.toc},
\DescPageRef{\LabelBase.option.toc}), sowie die Werte \PValue{totoc} und
