...
\IfThisCommonLabelBase{scrextend}{\iftrue}{\csname iffalse\endcsname}%
  ...
  \OptionValueRef{\ThisCommonLabelBase.option.extendedfeature}{title} gewählt
...
\IfThisCommonFirstRun{\iftrue}{%
  ...
  \autopageref{xmpl:\ThisCommonFirstLabelBase.env.titlepage}%
  \csname iffalse\endcsname%
}%
  \begin{Example}
    \phantomsection\label{xmpl:\ThisCommonLabelBase.env.titlepage}%
...
\IfThisCommonFirstRun{\iftrue}{%
  ...
  \autopageref{xmpl:\ThisCommonFirstLabelBase.cmd.extratitle}%
  \csname iffalse\endcsname%
}%
  \begin{Example}
    \phantomsection\label{xmpl:\ThisCommonLabelBase.cmd.extratitle}%
...
\PName{Untertitel}\ChangedAt{v2.97c}{\Class{scrbook}\and \Class{scrreprt}\and
  \Class{scrartcl}} wird  knapp unter dem Titel
ausgegeben.
...
vorhandenen Fußnoten ausgegeben wird.
...
\IfThisCommonLabelBase{scrextend}{%
  Damit die Schrifteinstellung für das Element \FontElement{footnote} beim
  Paket \Package{scrextend} Beachtung findet muss allerdings nicht nur die
...
%  \addtokomafont{caption}{\raggedright}%
    \begin{captionbeside}
      ...
      \begin{tabular}[t]{ll}
        ...
        \FontElement{title} & \DescRef{\ThisCommonLabelBase.cmd.usekomafont}\PParameter{disposition} \\
...
\IfThisCommonFirstRun{\iftrue}{%
  ...
  \autopageref{xmpl:\ThisCommonFirstLabelBase.maintitle}.%
  \csname iffalse\endcsname%
}%
  \begin{Example}
    \phantomsection\label{xmpl:\ThisCommonFirstLabelBase.maintitle}%
...
\begin{Explain}
  ...
  \OptionValueRef{\ThisCommonLabelBase.option.titlepage}{firstiscover}%
...
Element \FontElement{dedication}\IndexFontElement[indexmain]{dedication}, die
...
\IfThisCommonFirstRun{\iftrue}{%
  Ein Beispiel mit allen von \KOMAScript{} angebotenen Elementen für die
  Titelseiten finden Sie in \autoref{sec:\ThisCommonFirstLabelBase.titlepage}
  auf \autopageref{xmpl:\ThisCommonFirstLabelBase.fulltitle}.%
  \csname iffalse\endcsname%
}%
  \begin{Example}
    \phantomsection\label{xmpl:\ThisCommonFirstLabelBase.fulltitle}
