...
\autoref{tab:maincls.section.defaults}. Es ist zu beachten, dass dabei sowohl
\PValue{1ex} als auch \Length{baselineskip} jeweils von der voreingestellten
Größe der Überschrift oder des Inhaltsverzeichniseintrags abhängig ist.
...
\PValue{beforeskip} ergibt. Entsprechend setzt es diese Länge vor dem Aufruf
von \Macro{chapterheadendvskip} auf den Wert, der sich aus der
...
\Macro{RedeclareSectionCommand} konfigurierbar sein, so sollte man dafür
weiterhin \Length{@tempskipa} verwendet werden.
...
\end{lstcode}
Auch hier wird \Length{@tempskipa} vor der Verwendung der Befehle innerhalb
von \Macro{part} zuvor entsprechend der Einstellungen von
