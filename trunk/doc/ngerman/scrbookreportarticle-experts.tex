...
\Macro{IfIsAliaskomafont}\ChangedAt{v3.25}{\Class{scrbook}\and
  \Class{scrreprt}\and \Class{scrartcl}\and \Class{scrlttr2}} denn
...
\Macro{\PName{Name}} zu definiert beziehungsweise einen vorhandenen
...
\item[\PValue{section}] ist der Stil\textnote{Gliederungsstil
  ...
  Sternformen automatisch zusammen mit \DescRef{maincls.cmd.section}
  umkonfiguriert und kann nicht unabhängig davon verändert werden. Bei der
  Definition eines Gliederungsbefehls in diesem Stil wird gleichzeitig ein
  gleichnamiges Element angelegt, dessen Schrifteinstellung mit
...
Zu jeder Gliederungsanweisung gehört außerdem ein Zähler \iffree{mit
  demselben}{\unskip} \PName{Name}, der bei Bedarf von allen vier Befehlen mit
\Macro{newcounter} neu angelegt wird. Dasselbe gilt für die Ausgabe des
Zählers, \Macro{the\PName{Name}}, die Formatierung des Zählers,
...
\DescRef{maincls-experts.cmd.setparsizes} eingestellt Absatzeinzug.
\EndIndexGroup
...
\important{\PValue{increaselevel}}\OptionValue{increaselevel}{Ganzzahl}.
...
der \PName{Namensliste} dient. Für alle weiteren Gliederungsbefehle werden die
...
Diese\important[i]{\DescRef{maincls.cmd.chapter}\\
  ...
  \DescRef{maincls.cmd.addpart}} Anweisungen werden innerhalb der Definition
der Überschriften \DescRef{maincls.cmd.chapter}\IndexCmd{chapter},
...
verwendet. Dabei ist \Macro{chapterheadstartvskip} eine Anweisung, die dafür
vorgesehen ist, vor der Kapitelüberschrift einen vertikalen Abstand
einzufügen. Entsprechend ist \Macro{chapterheadendvskip} eine Anweisung, die
dafür vorgesehen ist, nach der Kapitelüberschrift einen vertikalen Abstand
...
\end{lstcode}
Auch hier wird von \DescRef{maincls.cmd.part}
\Length{@tempskipa}\IndexLength{@tempskipa} vor der Verwendung der Befehle
entsprechend der Einstellungen von
...
Anweisung für einige Dinge selbst verantwortlich. Dazu gehört neben dem
...
Das erste Argument, \PName{Ebene}, wird von
...
\Macro{scr@startsection} anstelle von \Macro{secdef} und
...
% LocalWords:  Gliederungsebene Eintragsebenen Absatzeinzug
