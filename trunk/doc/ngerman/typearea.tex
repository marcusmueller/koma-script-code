...
\end{lstcode}
  mit \OptionValue{BCOR}{8.15mm} als Option für \Package{typearea} oder
\begin{lstcode}
...
Verwenden Sie in diesem Fall an Stelle von
...
\begin{Explain}
  ...
  Rändern stattdessen mit dem symbolischen Wert \PValue{current} für
...
\begin{Explain}%
  ...
  vor. %
  \iffree{%
    Wird direkt aus dem DVI-Format in eine Low-Level-Druckersprache wie
    PCL%
    \iftrue% Umbruchkorrektur
  ...
  Ausgaben der 0-Bezugspunkt wie bei DVI links oben liegt. \iffree{Wird
    aber}{Heutzutage wird aber meist} in
  ...
  Papierformat in der Ausgabedatei angegeben werden sollte\iffree{,
    so}{. Dabei} fehlt diese Information. Als Lösung des Problems verwendet
  der entsprechende Treiber eine voreingestellte Papiergröße, die der Anwender
  entweder per Option oder durch entsprechende Angabe in der {\TeX}-Quelldatei
  verändern kann. Bei Verwendung des DVI-Treibers \File{dvips} oder
  \File{dvipdfm} kann diese Angabe in Form einer \Macro{special}-Anweisung
  erfolgen. Bei {pdf\TeX}, {lua\TeX}, {\XeTeX} oder {V\TeX} werden stattdessen
  deren Papierformat-Längen entsprechend gesetzt.
\end{Explain}
...
\begin{table}
  ...
  \begin{desctabular}
    \pventry{auto}{Falls die pdf\TeX-spezifischen Register
      ...
      geeignet.%
      \IndexOption{pagesize~=\PValue{auto}}}%
    ...
    \pventry{dvipdfmx}{\ChangedAt{v3.05a}{\Package{typearea}}Die Papiergröße
      ...
      \IndexOption{pagesize~=\PValue{dvipdfmx}}}%
    \pventry{dvips}{Bei Verwendung innerhalb der Dokumentpräambel wird die
      ...
      \IndexOption{pagesize~=\PValue{dvips}}}%
    \entry{\PValue{pdftex}, \PValue{luatex}}{%
      ...
      möglich.%
      \IndexOption{pagesize~=\PValue{pdftex}}%
      \IndexOption{pagesize~=\PValue{luatex}}}%
  \end{desctabular}
...
Korrektor trotzdem entgegenkommen. Dazu zählen dann Randeinstellungen, die für
einseitigen Druck mit einer Schreibmaschine zu brauchbaren Zeilenlängen
