...
\ProvidesFile{common-21.tex}[2013/10/16 KOMA-Script guide (common paragraphs)]
...
Argument \PName{string} unverändert ausgibt. Diese Voreinstellung kann jedoch
...
Aufruf von \Macro{automark} oder \Macro{manualmark} teilweise neu, um so
die gewünschten automatischen oder manuellen Kolumnentitel zu erreichen.%
...
resultierende linke Marke in \Macro{leftmark} die letzte auf der
entsprechenden Seite gesetzte Marke ist, während die resultierende rechte
Marke in \Macro{rightmark} die erste auf der entsprechenden Seite gesetzte
Marke ausgibt. Näheres dazu ist den weiterführenden Erklärungen zu
\Macro{rightmark} in \autoref{sec:maincls-experts.addInfos},
...
Gliederungsnummern in automatischen Kolumnentiteln mit Ausnahme der
Teile-Ebene und der Kapitel-Ebene verwendet. Dabei verwendet die Anweisung in
der Voreinstellung \Macro{@seccntmarkformat}, wenn eine solche Anweisung wie
...
bereitgestellt wird. All diesen Anweisungen gemeinsam ist, dass sie als
Argument den Namen der Gliederung, also beispielsweise \PValue{chapter} oder
\PValue{section} \emph{ohne} vorangestellten umgekehrten Schrägstrich
(engl. \emph{backslash}) erwarten.
...
% ToDo: Hier noch ein Beispiel einfügen!
