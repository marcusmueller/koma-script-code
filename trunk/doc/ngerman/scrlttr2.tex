...
\begin{Explain}
  ...
  gibt Register für Token, für Boxen, für Zähler, für Abstände (englisch:
  \emph{skip}) und für Größen (englisch: \emph{dimension}). Von all diesen
  Registern gibt es jeweils 256~Stück. Für \LaTeX{}-Längen, die mit
  \Macro{newlength} angefordert werden, werden Abstandsregister belegt. Sind
  alle diese Register verbraucht, kann man keine weiteren Längen
...
\begin{desclist}
  ...
  \pentry{fromrulethickness}{%
    Dicke einer optionalen \iffalse horizontalen \fi% Umbruchkorrektur
    Linie im Briefkopf (\autoref{sec:\LabelBase.firstHead},
    \DescPageRef{\LabelBase.plength.fromrulethickness})%
  }%
  \pentry{fromrulewidth}{%
    Länge einer optionalen \iffalse horizontalen \fi% Umbruchkorrektur
    Linie im Briefkopf (\autoref{sec:\LabelBase.firstHead},
...
\DescPageRef{\LabelBase.plength.firstfootvpos}. Als
...
vertikalen Marke sind über Pseudolängen konfigurierbar (siehe ab
\DescPageRef{\LabelBase.plength.foldmarkvpos}).
...
\DescRef{\LabelBase.option.foldmarks}\important{\DescRef{\LabelBase.option.foldmarks}}%
...
von der Option \DescRef{\LabelBase.option.foldmarks}%
...
der Anweisung \DescRef{\LabelBase.cmd.opening}\IndexCmd{opening} ausgegeben.
\iffalse% Umbruchkorrektur
...
über die Pseudolänge \DescRef{\LabelBase.plength.fromrulewidth}\IndexPLength{fromrulewidth}
...
\DescRef{\LabelBase.option.fromrule}\IndexOption{fromrule} in
\autoref{sec:\LabelBase.firstpage}, \DescPageRef{\LabelBase.option.fromrule}
...
\DescPageRef{\LabelBase.cmd.setplength}) in eigenen \File{lco}-Dateien
...
Ausrichtung wird mit Hilfe der Pseudolängen \DescRef{\LabelBase.plength.specialmailindent} und
...
Port-Payé-Anschrift über die Pseudolängen \DescRef{\LabelBase.plength.toaddrindent} (siehe
\DescPageRef{\LabelBase.plength.toaddrindent}) sowie
\DescRef{\LabelBase.plength.PPheadwidth} und \DescRef{\LabelBase.plength.PPheadheight} (siehe
\DescPageRef{\LabelBase.plength.PPheadheight}) bestimmt. Für die
Anordnung der Data-Matrix ist die Pseudolänge \DescRef{\LabelBase.plength.PPdatamatrixvskip}
(siehe \DescPageRef{\LabelBase.plength.PPdatamatrixvskip})
zuständig.
...
\Option{addrfield} ab (siehe \autoref{sec:\LabelBase.firstpage},
...
\DescRef{\LabelBase.option.addrfield} ab.%
\EndIndexGroup
...
\autoref{sec:\LabelBase.firstpage}, \DescPageRef{\LabelBase.option.addrfield}) und
\DescRef{\LabelBase.option.backaddress} (siehe \autoref{sec:\LabelBase.firstpage},
...
Versandart\Index{Versandart} gesetzt werden\iffree{. Diese wird genau dann
  gesetzt}{}, wenn die Variable \DescRef{\LabelBase.variable.specialmail} einen
...
\autoref{sec:\LabelBase.firstpage},
\DescPageRef{\LabelBase.option.refline}). Gleichzeitig wird dann auch
...
Geschäftszeile also nicht von der Option \DescRef{\LabelBase.option.refline}
...
\autoref{sec:\LabelBase.firstpage}, \DescPageRef{\LabelBase.option.subject}) die
...
\DescRef{\LabelBase.plength.sigbeforevskip}\IndexPLength{sigbeforevskip} (siehe
\DescPageRef{\LabelBase.plength.sigindent}) bestimmt.  Durch den Befehl
...
gesetzt.  Damit hat dann auch \DescRef{\LabelBase.option.enlargefirstpage}%
\important{\DescRef{\LabelBase.option.enlargefirstpage}} keine Wirkung. Ab
...
(siehe \autoref{sec:\LabelBase.firstpage},
\DescPageRef{\LabelBase.option.firstfoot}) abgeschaltet sein, so wird die
