...
\ProvidesFile{scrdatetime.tex}[{2010/03/11 KOMA-Script guide (chapter:
  scrdate, scrtime)}]
...
nun den Wochentag jedes beliebigen Datums im Gregorianischen Kalender
bestimmen.
...
\begin{Example}
  Sie wollen die Nummer des Wochentags des 24.~Dezembers~2027 wissen.
\begin{lstcode}
  Der 24.~Dezember~2027 hat die Wochentagsnummer 
  \the\ISODayNumber{2027-12-24}.
\end{lstcode}
  Als Ergebnis erhalten Sie:
  \begin{quote}
    Der 24.~Dezember~2027 hat die Wochentagsnummer 
...
\begin{Example}
   Sie wollen den Wochentags des 24.~Dezembers~2027 wissen.
\begin{lstcode}
  Der 24.~Dezember~2027 ist ein 
  \ISODayName{2027-12-24}.
\end{lstcode}
  Als Ergebnis erhalten Sie:
  \begin{quote}
    Der 24.~Dezember~2027 ist ein \ISODayName{2027-12-24}.
  \end{quote}
\end{Example}
...
Anweisungen \Macro{ISOToday} und \Macro{IsoToday} liefern direkt das aktuelle
Datum in ISO-Schreibweise. Sie unterscheiden sich lediglich darin, dass
\Macro{ISOToday} Monat und Tag immer zweistellig ausgibt, während
\Macro{IsoToday} Monat und Tag bei Werten kleiner 10 einstellig ausgibt. Die
Anweisung \Macro{todaysname} bietet direkt den aktuellen Wochentag, während
\Macro{todaysnumber} den Wert des aktuellen Wochentags liefert. Näheres zur
Verwendung dieses Wertes ist den obigen Erklärungen zu den Anweisungen
\Macro{DayNumber} und \Macro{ISODayNumber} zu entnehmen.
