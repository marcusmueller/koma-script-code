...
% Date of translated german file: 2007-07-24
...
contents, therefore the explanantion is given here.
...
similar to the standard commands \Macro{chapter} und
...
class \Class{scrartcl} but only in the classses \Class{scrbook} and
\Class{scrreprt}.
...
\Option{pointednumbers} und \Option{pointlessnumbers} and optionally
...
language is ouput instea, followed by \PName{additional text}. If the
...
front of the actual footenote. This is done with
...
describeded in \autoref{sec:maincls.font}. The respective elements for
...
necessary. However, its definition using \Macro{global} is quite
tricky. Most users would probably not be able to define such an
environment without help. Thus, as this definition can be very useful,
it was introduced in the above example.
...
strut to produce an example figure with a greater vertical height.
\end{Example}
%
