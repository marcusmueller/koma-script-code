...
% Date of translated german file: 2014-03-12
...
\Class{scrartcl}\OnlyAt{\Class{scrartcl}}. Because of this, you should not use
\Macro{partnumdepth}\IndexCmd{partnumdepth} for the value of
\Counter{tocdepth}.%
\begin{Example}
...
This is controlled by the {\LaTeX} counter \Counter{secnumdepth}. 
...
\begin{Example}
  ...
  have numbers. For this, you have to set counter \Counter{secnumdepth} inside
  the document preamble to the value represented by \Macro{sectionnumdepth}:
\begin{lstcode}
  \setcounter{secnumdepth}{\sectionnumdepth}
\end{lstcode}
\end{Example}
Redefinition of the commands is not allowed. Doing so could result in
unexpected results not even with \KOMAScript{} but third party packages
too. So it is recommended to never redefine any of them.

Please refer also to the explanation concerning the counter \Counter{tocdepth}
in \autoref{sec:maincls.toc},
\autopageref{desc:maincls.counter.tocdepth}. Depending on the class the values
of \Counter{tocdepth} and \Counter{secnumdepth} do not correspond with the
same section level.%
...
After\ChangedAt{v3.12}{\Class{scrbook}\and \Class{scrreprt}\and
  \Class{scrartcl}} describing above how to define the section level down to
which headings have numbers, these commands here can be used to execute code
depending on the fact, whether or not a \PName{section level} is numbered. If
with current setting of \Counter{secnumdepth} a \PName{section level} will be
numbered, then \Macro{ifnumbered} executes the \PName{then code}. In opposite
to this, \Macro{ifunnumbered} executes the \PName{else code}. If with current
setting of \Counter{secnumdepth} a \PName{section level} will not be numbered,
then \Macro{ifnumbered} executes the \PName{else code}. In opposite to this,
\Macro{ifunnumbered} executes the \PName{then code}. The \PName{section level}
is simply a name like \PValue{part}, \PValue{chapter}, \PValue{section},
...
\KOMAScript{} itself uses these tests, e.\,g., for the definition of
\Macro{chaptermark}\IndexCmd{chaptermark} within page style
\Pagestyle{headings}\IndexPagestyle{headings}. This indirectly guaranties that
headings of \Macro{addchap} do not set a number inside the running head (see
also \Macro{addchapmark}, \autopageref{desc:maincls.cmd.addchapmark}).%
