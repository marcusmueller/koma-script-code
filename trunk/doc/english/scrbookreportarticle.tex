...
\ProvidesFile{scrbookreportarticle.tex}[2012/02/14 KOMA-Script guide (chapter:
...
left-justified. This option may be used at the optional argument of
\Macro{documentclass} but not at the argument of \Macro{KOMAoptions} or
...
The standard classes format titles of floating environments, that are placed
...
But at tables often headings are used. That's because there may be tables that
span several pages. Surely the reader wants to know the purpose of the table
at the first page already. Furthermore tables will be read row by row from top
down to bottom. So there are at least two good reasons to generally use table
headings. \KOMAScript{}\important{\OptionValue{captions}{tableheading}}
therefor supports option
\OptionValue{captions}{tableheading}%
\IndexOption{captions~=\PValue{tableheading}}, that changes the caption format
into headings at tables only.

Please note\textnote{Attention!} that multi-page tabulars may not use any
floating environment. To have an automatic page break at any kind tabular you
...
\IndexOption{captions~=\PValue{tablesignature}}. Note\textnote{Attention!} that using any of these options does not change the position of
...
Of course similar feature exists\ChangedAt{v3.09}{\Class{scrbook}\and
...
\IndexOption{captions~=\PValue{figuresignature}}. Nevertheless figures like
photos will be viewed at a whole and in case of a diagram or graph mostly
examined from
left bottom to the right. Therefor in general signatures should be used and it
wouldn't be useful to change the caption format from signatures to headings.
