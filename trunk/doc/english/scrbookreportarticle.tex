...
in the extended optional argument. In opposite to the later explained
\hyperref[desc:maincls.cmd.part*]{starred versions of the sectioning commands}
the titles will still be added to the table of contents and, if indicated,
used for the running head. For \Macro{part}, \Macro{chapter} und
\Macro{section} using \OptionValue{nonumber}{true} would be similar to the
usage of the commands \DescRef{\LabelBase.cmd.addpart},
\DescRef{\LabelBase.cmd.addchap} and \DescRef{\LabelBase.cmd.addsec}, which
are explained on \DescPageRef{\LabelBase.cmd.addpart}.
% :ODOT
...
contents.
% TODO: Fix new translation
These commands also support the extension of the optional argument triggered
by option \DescRef{\LabelBase.option.headings}%
\important{\DescRef{\LabelBase.option.headings}}, but switching on or off
\Option{nonumber} does not have any effect. 
% :ODOT
