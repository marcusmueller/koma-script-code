...
\translator{Jens-Uwe Morawski\and Gernot Hassenpflug\and Markus Kohm\and
  Krickette Murabayashi\and Jana Schubert}
...
\begin{Example}
  For a book project the section levels from part own to section should be
  enumerated. This can be achieved by setting the counter
  \Counter{secnumdepth} inside the document preamble to the value represented
  by \Macro{sectionnumdepth}:
\begin{lstcode}
  \setcounter{secnumdepth}{\sectionnumdepth}
\end{lstcode}
\end{Example}
Redefinition of the commands is not allowed, as it could lead to severe
interference with \KOMAScript{} and third party packages. In fact you should
never redefine any of them.
...
of \Counter{tocdepth} and \Counter{secnumdepth} do not correspond to the
same section level.%
...
numbered, then \Macro{ifnumbered} executes the \PName{then code}. In contrast
...
then \Macro{ifnumbered} executes the \PName{else code}. In contrast to this,
