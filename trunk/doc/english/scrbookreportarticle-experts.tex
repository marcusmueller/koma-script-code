...
\item[\PValue{chapter}:] \ChangedAt{v3.18}{\Class{scrbook}\and
  ...
  \DescRef{maincls.cmd.addchap}\IndexCmd{addchap}. You can define new section-like commands
  ...
  Note\textnote{Attention!} that command \DescRef{maincls.cmd.addchap} and the star variants
  are configured automatically together with \DescRef{maincls.cmd.chapter} and should not be
...
\item[\PValue{part}:] \ChangedAt{v3.18}{\Class{scrbook}\and
  ...
  currently used for \DescRef{maincls.cmd.part}\IndexCmd{part} and indirectly for
  \DescRef{maincls.cmd.addpart}\IndexCmd{addpart}. You can define new section-like commands
  using this style. To define new or to reconfigure existing commands you can
  also use the additional attributes of
  \autoref{tab:maincls-experts.declarepartstyle.keys},
  \autopageref{tab:maincls-experts.declarepartstyle.keys}.
  Note\textnote{Attention!} that command \DescRef{maincls.cmd.addpart} and the star variants
  are configured automatically together with \DescRef{maincls.cmd.part} and should not be
...
\item[\PValue{section}:] Style of section headings. This style is currently
  ...
  \DescRef{maincls.cmd.subparagraph}\IndexCmd{subparagraph}. You can define new section-like
  ...
  with \DescRef{maincls.cmd.section} and should not be changed independently. Defining a new
  ...
  \DescRef{maincls.cmd.setkomafont} or \DescRef{maincls.cmd.addtokomafont} (see
  \autoref{sec:maincls.textmarkup},
  \DescPageRef{maincls.cmd.setkomafont}), if such an element does not
  already exist.
\end{labeling}
...
\begin{table}
  ...
  \begin{tabularx}{\linewidth}{llX}
    ...
    \PValue{prefixfont} & \PName{font commands} &
                                          ...
                                          for \DescRef{maincls.cmd.setkomafont} and
                                          \DescRef{maincls.cmd.addtokomafont} for the
...
\begin{Example}
  Because of incomprehensible reasons, the headings of \DescRef{maincls.cmd.paragraph}
  shouldn't be run-in headings any longer but headings similar to
  \DescRef{maincls.cmd.subsubsection}. The vertical skip above the heading should be 10\,pt
...
needed, because since v3.15 \KOMAScript{} defines \DescRef{maincls.cmd.paragraph} itself
...
Some settings of \DescRef{maincls.cmd.chapter} depend on option \DescRef{maincls.option.headings} (see
...
\begin{table}
  ...
  \begin{tabular}{ll}
    \multicolumn{2}{@{}l}{\bfseries With \OptionValueRef{maincls}{headings}{big}:}\\
    \toprule
    ...
    \bottomrule\\
    \multicolumn{2}{@{}l}{\bfseries With \OptionValueRef{maincls}{headings}{normal}:}\\
    \toprule
    Attribute & Default Value \\
    \midrule
    \PValue{afterskip}
    ...
    \bottomrule\\
    \multicolumn{2}{@{}l}{\bfseries With \OptionValueRef{maincls}{headings}{small}:}\\
    \toprule
    Attribute & Default Value \\
    \midrule
    \PValue{afterskip}
...
\end{lstcode}
Again \DescRef{maincls.cmd.part} sets \Length{@tempskipa}\IndexLength{@tempskipa} to the
...
implement the options \DescRef{maincls.option.appendixprefix} (see
\autoref{sec:maincls.structure},
\DescPageRef{maincls.option.appendixprefix}). You\textnote{Attention!}
