...
% Date of the translated German file: 2019-08-07
...
their own classes based on a \KOMAScript{} class.
% TODO: Fix new translation
Because such descriptions are not addressed to beginners, they are condensed
and require deeper knowledge in \LaTeX.
% :ODOT
Other parts describe features that exist only for the sake of compatibility
with the standard classes or earlier versions of \KOMAScript{}.\iffree{
...
of little or no interest to the average user. Concerning the currently used
\KOMAScript{} release, have a look at \DescRef{scrbase.cmd.KOMAScriptVersion}
...
% TODO: Fix new translation
\KOMAScript{} classes provide additional features in generation and
manipulation of entries to the table of contents. Some of them base upon
package \Package{tocbasic} (see \autoref{sec:tocbasic.tocstyle} from
\autopageref{sec:tocbasic.tocstyle}). Other are implemented in the classes.
% :ODOT
...
% TODO: Fix new translation
\KOMAScript{} classes do not only provide an extended selection of basic font
sizes. They also offer the definition of elements with individual font
settings and the manipulation and dedicated usage of these font settings.
% :ODOT
...
% TODO: Fix new translation
Not only but especially because of the \KOMAScript{} classes' extended
features in changing the paragraph indention or gap, direct changes of the
standard lengths \Length{parskip}\IndexLength{parskip},
\Length{parindent}\IndexLength{parskip} and
\Length{parfillskip}\IndexLength{parskip} should mostly be avoided.
% :ODOT
...
% TODO: Fix new translation
The \KOMAScript{} classes provide extensive features to influence sectioning
levels and the corresponding headings. Even the definition of new levels is
feasible.
% :ODOT
...
\end{Declaration}
% TODO: improve new translation
In\ChangedAt{v3.27}{\Class{scrbook}\and \Class{scrreprt}\and \Class{scrartcl}}
some cases it could be useful to be able to test, whether or not a sectioning
command has a dedicated \PName{style}. If \KOMAScript{} has defined the
...
executed. An error is reported if \Macro{\PName{name}} is undefined or not a
\KOMAScript{} sectioning command.%
% :ODOT
\EndIndexGroup
...
\end{Declaration}
\BeginIndex{}{hooks}%
Beside\ChangedAt[2019/01]{v3.27}{\Class{scrbook}\and \Class{scrreprt}\and
  \Class{scrartcl}} all their attributes,
...
sectioning commands, to be manipulated with
...
It is important\important{Attention!} to avoid adding code to the hooks, that
would influence the page breaking or the position of the heading. Therefore
only advanced users should use these hook. In case of doubt you should even
consult the code of the class for information about when a hook is
executed. These hooks are a kind of backstop before you indeed would redefine
a section command without using \KOMAScript{} features.

The\textnote{\DoHHook{\dits/preinit/\dots}} hook
\DoHook{heading/preinit/\PName{name}} is executed immediately before any
initialization of the sectioning command. At this point no settings have been
done. Even finishing of the paragraph before the heading has not yet been
forced.

The\textnote{\DoHook{\dots/postinit/\dots}} hook
\DoHook{heading/postinit/\PName{name}} is executed somehow later. At this
point some of the settings are done and the previous paragraph has been
printed.

Only\textnote{\DoHook{\dots/branch/\dots}} one of the hooks
\DoHook{heading/branch/nostar/\PName{name}} or
\DoHook{heading/branch/star/\PName{name}} is executed, after detecting whether
the star variant or the normal variant of a section command is used. At this
point also vertical gaps above the heading are already inserted.

The\textnote{\DoHook{\dots/begingroup/\dots}} hook
\DoHook{heading/begingroup/\PName{name}} is executed just at the beginning of
the group used to process the heading. This is also the last occasion to
invent before the heading is printed.

Analogous\textnote{\DoHook{\dots/\begingroup/\dots}}, the hook
\DoHook{heading/begingroup/\PName{name}} is executed at the end of the group
used to process the heading. Currently this is the last hook inside section
commands.

Please note\textnote{\DescRef{maincls.cmd.minisec}},
\DescRef{maincls.cmd.minisec} is not a real sectioning command and therefore
all these hooks are not available for it.%
\EndIndexGroup
% :ODOT
...
Strictly\ChangedAt{v3.27}{\Class{scrbook}\and \Class{scrreprt}\and
  \Class{scrartcl}} speaking, this is an internal command. It is specified
only inside sectioning commands from hook \DoHook{\dots/begingroup/\dots} till
\DoHook{\dots/begingroup/\dots}. In this case \Parameter{then code} is
executed if and only if the sectioning heading should be numbered considering
the value of \DescRef{maincls.counter.secnumdepth} the non-star variant of a
sectioning command. If the heading should not be numbered either because of
using the star variant of a sectioning command or because of the
\DescRef{maincls.counter.secnumdepth} setting, the \PName{else code} is
executed. If the sectioning command uses the style \PValue{chapter} only
headings of the main matter are numbered and use \PName{then code}.

If you use the command outside a sectioning command, the result is
unspecified. In this case, usually it will result in an error message and
execute neither \PName{then code} nor \PName{else code}.%
\EndIndexGroup
% :ODOT
...
\item Instead of
  ...
  \KOMAScript{} uses \Macro{scr@startsection}. But the definition of
  \Macro{@startsection} is checked. If the implementation does not meet the
  expectation, a warning message is shown, several features of \KOMAScript{}
  are deactivated and \Macro{scr@startsection} uses a copy of
  \Macro{@startsection} whereas \Macro{@startsection} is redefined to use
  \Macro{scr@startsection}.
\item Instead of \Macro{@dblarg}\IndexCmd{@dblarg}\important{\Macro{@dblarg}}
  \KOMAScript{} uses an internal command in the definition of sectioning
  commands.
\item Instead of \Macro{secdef}\IndexCmd{secdef}\important{\Macro{secdef}}
  \KOMAScript{} uses \Macro{SecDef} to achieve the change of \Macro{@dblarg}
  mentioned above. If the implementation of \Macro{secdef} differs from the
  expected one, a warning message is shown.
\item \Macro{@sect}\IndexCmd{@sect}\important{\Macro{@sect}} is redefined to
  achieve several extensions of \KOMAScript.
\item \Macro{@ssect}\IndexCmd{@ssect}\important{\Macro{@ssect}} is redefined
  to achieve several extensions of \KOMAScript.
\item \Macro{@xsect}\IndexCmd{@xsect}\important{\Macro{@xsect}} is redefined
  to achieve several extensions of \KOMAScript.
\end{itemize}
There are thoughts to avoid changes of the \LaTeX{} kernel commands in
future, but to switch completely to new, proprietary internal commands. If
someone uses generally incompatible sectioning packages, one
effect would be to switch over the responsibility to sectioning commands
completely to these packages. On the other hand extra effort would be needed
to keep the compatibility of other packages.
...
\KOMAScript{}'s functionality.  The meaning of the parameters of these
...
offers the hook described before.%
\EndIndexGroup
% ODOT:
...
\begin{Explain}
  ...
  Until\ChangedAt[2019/01]{v3.27}{\Class{scrbook}\and \Class{scrreprt}\and
    \Class{scrartcl}} \KOMAScript~v3.26b these commands has been offered as an
  ...
  Internally \Macro{At@startsection} is realised with hook
  \DescRef{\LabelBase.dohook.heading/postinit}. \Macro{Before@ssect} is
  implemented by \DescRef{\LabelBase.dohook.heading/branch/star} and
  \Macro{Before@sect} uses
  \DescRef{\LabelBase.dohook.heading/branch/nostar}. The \PName{code} is added
  using \DescRef{scrbase.cmd.AddtoDoHook}. Removing code, that once has been
  added, is not provided.%
  \EndIndexGroup
\end{Explain}
% :ODOT
...
% TODO: Fix new translation
The descriptions of this section become less important with the usage of
packages like \Package{biblatex}. The extensive features of such packages
replace the extensions of the \KOMAScript{} classes then.
% :ODOT
