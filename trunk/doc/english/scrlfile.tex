...
% Date of translated german file: 2013/11/22
...
won't clean the \PName{instead code} of a package. Only the execution won't be
...
from a stored one won't reactivate the \PName{instead code} of the packages.%
%
\begin{Example}
  Assuming, you want to prevent a package \Package{foo} from being loaded, but
  you don't want an already stored \PName{instead code} to be
...
\end{lstcode}
  For \Macro{UnPreventPackageFromLoading} it doesn't matter whether or not
