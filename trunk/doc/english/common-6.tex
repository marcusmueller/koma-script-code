...
\ProvidesFile{common-6.tex}[2012/02/05 KOMA-Script guide (common paragraphs)]
...
\IfCommon{scrlttr2}{%
  ...
  prefered over paragraph spacing. But the elements the argumentation depends on,
  i.\,e., figures, tables, lists, equations, and even new pages, 
  are rare. Often letters are not so long, that a oversighted paragraph will
...
\ifIgnoreThis %+++++++++++++++++++++++++++++++++++++++++++++ nicht maincls +
\IfCommon{scrlttr2}{\ Apart vom this suggestions everthing, that war written
  ar \autoref{sec:maincls.parmarkup} for the other \KOMAScript{} class, ist
  valied for letters too.}
\else %------------------------------------------------------- nur maincls -
...
The \PName{manner} consists in two elements. The first element is either
...
Beside the resulting eight possible combinations for \PName{manner} the values
...
be same like using \PValue{full} without annex and therefor will result in
