...
sown in \autoref{tab:truefalseswitch} on \autopageref{tab:truefalseswitch}. If
...
the argument \PName{string} without changes. But the default may change either
redefining \Macro{MakeMarkcase} or using option \Option{markcase}, that
...
\Package{scrlayer} optionally provides this too. Therefore you'd use option
\OptionValue{markcase}{upper}. The bottom line of this is a redefinition of
...
decide, where to place the marks' contents. With \Macro{leftmark} you can put
the contents of the left mark.

Similar you can use \Macro{rightmark} to put the contents of the right
...
\Package{scrlayer} would either be \Macro{leftmark} or \Macro{rightmark}
depending on whether the current page is a even or odd one.

Strictly thought command \Macro{pagemark} isn't concerned with \TeX's mark
...
the output. This may be changed using command \Macro{setkomafont} or
...
\end{lstcode}
  ...
  Commands \Macro{ihead} and \Macro{ohead} has been used to place the wanted
...
\Macro{cfoot} in the example above it not the same like omitting the optional
...
they also support the \Macro{autodot} mechanism of the \KOMAScript{}
...
to also use counter \Counter{secnumdepth} for the test whether or not the
section level should output a number. This is even the case, if the user
doesn't change counter \Counter{secnumdepth} himself, because packages and
...
\begin{lstcode}
  ...
  \end{document}
\end{lstcode}
  Here command \Macro{MakeMarkcase} has been used to the same letter case the
  automatic running heads are using.

  Now, let's move command \Macro{tableofcontents} before the preface. Let's
  remove the \Macro{markboth} command too. You'll find, that the preface now
  has the running head ``\texttt{CONTENTS}''. This is because of a quirk of
  \Macro{chapter*} (see also  \autoref{sec:maincls.structure} on
...
\Macro{chapter} also for the section an subsection level, if the mark commands
...
\end{lstcode}
  ...
  defined \Macro{numberbox} using \Macro{DeclareRobustCommand*} instead of
