...
\ProvidesFile{common-14.tex}[2012/02/05 KOMA-Script guide (common paragraphs)]
...
In general we distinguish two kinds of document titles. First known are
title pages. In this case the document title will be placed together with
additional document information like the author on a page of it's own. Beside
the main title page several further title pages may exist like the half-title
or bastard title, publisher data, dedication or similar. The second known kind
of document title is the in-page title. In this case the document title is
placed on the top of a page specially emphasized and maybe together with some
addiotional information too, but it will be followed by more material in the
same page, for instance by an abstract or the table of contents or even a
section.
\fi %**************************************************** Ende nur maincls *

\IfCommon{scrextend}{%
  The\textnote{Attention!} document title capabillities of \Package{scrextend}
...
and in-page title\Index{title>in-page}. \PName{simple switch} any value from 
...
whole counting gets mixed up. In my opinion there are only two meaningful
...
%
With the standard classes, the back of the title page of an double-side print
