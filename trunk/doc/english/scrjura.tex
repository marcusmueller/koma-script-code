...
As a default, the paragraph number will be printed using the currently active
font. However, you can change this default of element \FontElement{parnumber}
using \DescRef{\LabelBase.cmd.setkomafont}%
\important[i]{\DescRef{\LabelBase.cmd.setkomafont},
  \DescRef{\LabelBase.cmd.addtokomafont}} and
...
\end{Declaration}
If the paragraph number is not printed, \Package{scrjura} executes command
\Macro{withoutparnumber} at the beginning if the new paragraph. The initial
definition of this command is empty. This means, it is a kind of dummy
command, that does nothing. It has been implemented because of a user
request. General users can ignore this command.%
\EndIndexGroup
...
\begin{desclist}
  ...
  {Options of \Macro{DeclareNewJuraEnvironmante} (\emph{continued})}%
  \entry{\OptionVName{Clause}{instruction}}{%
    Defines on which \PName{instruction} inside the environment the command
    \DescRef{\LabelBase.cmd.Clause} is being mapped. The \PName{instruction}
    expects exactly one argument. To use it correctly, a deeper knowledge and
    understanding of the internal functions of \Package{scrjura} is
    needed. Furthermore the requirements for \PName{instruction} may change in
    future versions. It is recommended not to use this option!%
  }%
  \entry{\OptionVName{ClauseFont}{instructions}}{%
    \leavevmode\BeginIndex{FontElement}{\PName{Name}.Clause}%
    \LabelFontElement{\PName{Name}.Clause}%
    If this option is used, a new element
    \FontElement{\PName{Name}.Clause}\IndexFontElement{\PName{Name}.Clause} is
    defined using
    \DescRef{maincls-experts.cmd.newkomafont}\IndexCmd{newkomafont} and
    \PName{instructions} as the initial settings of the new element. If the
    element has been an alias (see \DescRef{maincls-experts.cmd.aliaskomafont}
    in \autoref{sec:maincls-experts.experts},
    \DescPageRef{maincls-experts.cmd.aliaskomafont}), is becomes a standalone
    element now. If it already has been a standalone element,
    \DescRef{\LabelBase.cmd.setkomafont} is used to setup \PName{instructions}
    a new font settings. Please note the limitations for font setting in
    \autoref{sec:\LabelBase.textmarkup},
    \DescPageRef{\LabelBase.cmd.setkomafont}!
    \EndIndex{FontElement}{\PName{Name}.Clause}%
  }%
  \entry{\OptionVName{SubClause}{instruction}}{%
    Defines on which \PName{instruction} inside the environment the command
    \DescRef{\LabelBase.cmd.SubClause} is being mapped. The
    ...
    \PName{instruction} may change in future versions. It is recommended not
    to use this option!%
  }%
  \entry{\OptionVName{Sentence}{instruction}}{%
    Defines on which \PName{instruction} inside the environment the command
  ...
  \entry{\OptionVName{ClauseNumberFormat}{instruction}}{%
    Formats the numbers of clauses. The \PName{instruction} should expect
    exactly one argument: the number of the clause. If the \PName{instruction}
    implements a series of instructions and the number is the last argument of
    a that series, you may directly use the series of instructions as
    \PName{instruction}.%
  }
\end{desclist}
\begin{Example}
