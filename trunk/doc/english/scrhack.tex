...
% Date of the translated German file: 2019-09-26
...
Several\ChangedAt{v3.27}{\Package{scrhack}} packages act on the assumption,
...
on the definition of the standard classes. From release~3.27 \Package{scrhack}
...
\DescRef{maincls.cmd.subparagraph}\IndexCmd{subparagraph} compatible to the
standard classes. In the case of an existing \DescRef{maincls.cmd.chapter} the
definitions base on \Class{book}. In case of \DescRef{maincls.cmd.chapter}
does not exist, the definitions of \Class{article} are used.
...
\DescRef{maincls.cmd.partformat} change their settings.
...
package \Package{scrhack}. If you want to use it, you have to explicitly
activate it using option
\Option{standardsections}\IndexOption[indexmain]{standardsections} while
loading the package. Late activation after loading the package is not
supported.

Mostly there are less invasive solutions to fix the problem of package
...
% :ODOT (don't forget the % at the end of the last line)
\EndIndexGroup
