...
\IfThisCommonLabelBase{maincls}{%
  ...
  \IfThisCommonLabelBase{scrlttr2}{%
    ...
    refers, e.\,g., figures, tables, lists, equations, and even new pages, are
    rare in normal letters. Letters are usually not so long that an
    ...
    stands out from the crowd. Another is that it maintains the
    \emph{corporate identity}, that is the uniform appearance, of all
...
plus character as second element%
\important{\OptionValue{parskip}{\PName{distance}+}}, the white space will be
at least one third, and with the asterisk one fourth, the width of a normal
line. With the minus variant%
...
In addition to the resulting eight combinations for \PName{method}, the values
for simple switches shown in \autoref{tab:truefalseswitch},
\autopageref{tab:truefalseswitch} can be used. Switching on the option%
\important{\Option{parskip}\\\OptionValue{parskip}{true}}
corresponds to using \PValue{full} with no second element and therefore 
results in inter-paragraph spacing of one line with at least 1\Unit{em} 
white space at the end of the last line of each paragraph. Switching off%
\important{\OptionValue{parskip}{false}} the option re-activates the default
of 1\Unit{em} indent at the first line of the paragraph instead of paragraph
spacing. A summary of all possible values for \PName{method} are shown in
...
%    \caption
  ...
  \pventry{full-}{%
    ...
    may be arbitrarily filled\IndexOption{parskip~=\textKValue{full-}}}%
  \pventry{full+}{%
