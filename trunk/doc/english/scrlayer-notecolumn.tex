...
% Date of the translated German file: 2019-02-13
...
in notes. To provide such changes of the colour, we would need a colour
management for each note column using a kind of \emph{colour stack}. Due to
the fact, that the package should only be a kind of \emph{proof of concept},
the time and effort to implement such a colour management has been
circumvented by allowing colour changes only at the attributes of the font
element \FontElement{notecolumn.\PName{note column name}}.

The second issue is of more conceptional nature. The content of the auxiliary
file, that contains the note column information, is read while building the
page head. This is important if the page building is done in the context of an
environment like \Environment{verbatim}. In such a case the \Macro{catcode}
settings of the environment are active while reading the auxiliary file. This
inevitably would result in processing errors and wrong output.%
% End of TODO
\EndIndexGroup
