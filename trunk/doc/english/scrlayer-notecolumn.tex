...
The Package \Package{scrlayer-notecolumn} should solve all these problems. To
...
\begin{Declaration}
  ...
  \OParameter{option~list}\Parameter{note~column~name}%
\end{Declaration}
These commands can be used to create note columns. \Macro{DeclareNoteColumn}
...
simply does nothing in that case. \Macro{RedeclareNoteColumn} can be used only
...
element\textnote{element name} \FontElement{notecolumn.marginpar}. The initial
setting of the element's font can be directly specified when declaring a note
column by using the \Option{font} option within the \PName{option list}.%
...
\begin{desclist}
  \desccaption{%
%  \caption[Possible settings for declaring note columns]{%
    Possible settings for declaring note columns%
%  }%
