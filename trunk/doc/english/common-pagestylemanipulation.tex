...
\IfThisCommonLabelBase{scrlayer-scrpage-experts}{\iffalse}{%
  ...
  \IfThisCommonLabelBase{scrlayer-scrpage}{\iftrue}{\csname
    iffalse\endcsname}%
    \begin{Example}
      \phantomsection\label{xmpl:\ThisCommonLabelBase.mark}%
...
\end{lstcode}
  ...
  \IfThisCommonLabelBase{scrlayer-scrpage}{\iftrue}{\csname
    iffalse\endcsname}%
    \begin{Example}
      \phantomsection\label{xmpl:\ThisCommonLabelBase.mark.oneside}%
...
\end{lstcode}
  ...
  \IfThisCommonLabelBase{scrlayer-scrpage}{%
    ...
    of \Macro{documentclass}), but you do not want the dimension lines, you
    can deactivate them for the package only using \OptionValue{draft}{false}
    as an optional argument of \Macro{usepackage} while loading the package.%
  }{}%
  \EndIndexGroup
  ...
  \Option{markcase}, that %
  \IfThisCommonLabelBase{scrlayer-scrpage}{will be described next}{%
    ...
    \begin{table}
      ...
      \begin{desctabular}
        \pventry{lower}{\IndexOption[indexmain]{markcase~=lower}%
          redefines \Macro{MakeMarkcase} to convert the automatic running
          heads into lower case letters using \Macro{MakeLowercase} (lower
          case typesetting).%
        }%
        \pventry{upper}{\IndexOption[indexmain]{markcase~=upper}%
          redefines \Macro{MakeMarkcase} to convert the automatic running
          heads into upper case letters using \Macro{MakeUppercase} (upper
          case typesetting).%
        }%
        \pventry{used}{\IndexOption[indexmain]{markcase~=used}%
          redefines \Macro{MakeMarkcase} to use automatic running heads
          without any case changes.%
        }%
        \entry{\PValue{ignoreuppercase}, \PValue{nouppercase},
          \PValue{ignoreupper},
          \PValue{noupper}}{\IndexOption[indexmain]{markcase~=noupper}%
          redefines not only \Macro{MakeMarkcase} but also locally to the
          running heads \Macro{MakeUppercase} and \Macro{uppercase} to leave
          the automatic running heads unchanged.%
        }%
      \end{desctabular}
    \end{table}
  }{}%
  ...
  \IfThisCommonLabelBase{scrlayer-scrpage}{\iftrue}{%
    ...
    \autopageref{xmpl:scrlayer-scrpage.cmd.headmark}. The package
    \Package{scrlayer-scrpage} internally uses a lot of such features of
    \Package{scrlayer}.%
    \csname iffalse\endcsname}%
    \begin{Example}
      \phantomsection\label{xmpl:\ThisCommonLabelBase.cmd.headmark}%
...
\end{lstcode}
  ...
  \IfThisCommonLabelBase{scrlayer-scrpage}{\iftrue}{%
    ...
    \autopageref{xmpl:scrlayer-scrpage.cmd.sectionmarkformat} for more
    information.%
    \csname iffalse\endcsname%
  }%
    \begin{Example}
      \phantomsection\label{xmpl:\ThisCommonLabelBase.cmd.sectionmarkformat}%
...
\end{lstcode}
  ...
  to also use the counter \Counter{secnumdepth} for the test whether or not
  the section level should output a number, even in the case the user does not
  change counter \Counter{secnumdepth} himself, because packages and classes
  may do so locally and rely on correct handling of \Counter{secnumdepth}.
  ...
  \IfThisCommonLabelBase{scrlayer-scrpage}{\iftrue}{%
    ...
    \autoref{sec:scrlayer-scrpage.pagestyle.content},
    \autopageref{xmpl:scrlayer-scrpage.cmd.markboth}. Package
    \Package{scrlayer-scrpage} is a kind of extension of \Package{scrlayer}.%
    \csname iffalse\endcsname%
  }%
    \begin{Example}
      \phantomsection\label{xmpl:\ThisCommonLabelBase.cmd.markboth}%
...
\begin{lstcode}
  ...
  \end{document}
\end{lstcode}
      Using command \Macro{MakeMarkcase} results in getting the same letter
      case as for automatic running heads.
...
\IfThisCommonLabelBase{scrlayer-scrpage}{\iffalse}{\csname iftrue\endcsname}
  ...
  \IfThisCommonLabelBase{scrlayer-scrpage-experts}{\iftrue}{%
    ...
    \autopageref{xmpl:scrlayer-scrpage-experts.cmd.GenericMarkFormat}.%
    \csname iffalse\endcsname}%
    \begin{Example}
       \phantomsection
