...
The package \Package{scrbase} needs the functionality of the
...
\end{lstcode}
  after you define the key, you can specify the option \Option{coldcuts} when
...
\end{lstcode}
  ...
  e.\,g., using \DescRef{\LabelBase.cmd.FamilyExecuteOptions} before
...
of the unknown option. \KOMAScript{} itself uses this feature, e.\,g., inside
the definition of sectioning commands, to evaluate the style option before all
other attributes.
...
This can be done using \Macro{AtEndOfFamilyOptions} or its starred
...
example with option \Option{coldcuts}, the user had to assign either
...
\end{lstcode}
  ...
  \Macro{FamilySetBool}, its status can be checked with the help of
...
only the name of a command, e.\,g., \PParameter{saladweight} instead of
...
\Macro{\PName{name}}. Using option \Option{internalonly} without a
...
\begin{Declaration}
  ...
  \Parameter{then code}\Parameter{else code}
\end{Declaration}
If\ChangedAt{v3.21}{\Package{scrbase}} lua\TeX{} is used, the \PName{then
code} will be executed. Otherwise, the \PName{else code} will be executed.
...
\end{Declaration}
If\ChangedAt{v3.12}{\Package{scrbase}} \PName{expression} expands to something
syntactically equivalent to of the value of a counter, i.\,e., a a negative or
...
Package \Package{scrbase} provides them.
%
\EndIndexGroup
...
\section{Extensions of the Mathematical Features of \eTeX}
\seclabel{etex}
...
The value can be used to assign to a counter or directly within an expression
using \Macro{numexpr}. To output the value as an Arabic number, you must
prefixed it with \Macro{the}.%
%
\EndIndexGroup
%
\EndIndexGroup
