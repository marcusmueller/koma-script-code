...
These two commands differ from \DescRef{\LabelBase.cmd.FamilyBoolKey}
respectively \DescRef{\LabelBase.cmd.FamilySetBool} only in the inverse
boolean logic they are using. Inverse logic means, the values \PValue{true},
\PValue{yes}, and \PValue{on} set the boolean switch given by
\PName{switch name} to \Macro{iffalse} and therefore deactivate it. The values
\PValue{false}, \PValue{no}, and \PValue{off} set the boolean switch to
\Macro{iftrue} and therefore active it.%
% :ODOT
\EndIndexGroup
