...
% Date of the translated German file: 2018-02-16
...
\begin{Declaration}
  \Macro{FamilyStringKey}\OParameter{family member}\Parameter{family}%
                         \Parameter{key}%
                         \OParameter{default}\Parameter{command}
  \Macro{FamilyCSKey}\OParameter{family member}\Parameter{family}%
                         \Parameter{key}%
...
the same as:
\begin{lstcode}[escapeinside=`']
  \DefineFamilyKey[`\PName{family member}']{`\PName{family}'}{`\PName{key}'}
                  {\def`\PName{command}'{#1}}
\end{lstcode}
...
command \Macro{FamilyCSKey} does not expect a command as last argument but
