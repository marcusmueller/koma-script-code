...
\IfThisCommonLabelBase{maincls}{%
  ...
  \IfThisCommonLabelBase{scrlttr2}{%
    The basic commands to set footnotes can be found in any introduction to
    \LaTeX, e.\,g., at \cite{lshort}. \KOMAScript{}%
...
\IfThisCommonFirstRun{%
  .
  \begin{table}
    \caption[{Possible values for the \Option{footnotes} option}]
    {Possible values for the \Option{footnotes} option to configure footnotes}
...
\IfThisCommonFirstRun{\iftrue}{%
  Examples and additional hints can be found in
...
\end{lstcode}
  ...
  \IfThisCommonLabelBaseOneOf{scrlttr2,scrextend}{\par%
    An example of how to use \Macro{footref} can be found in
...
\FontElement{footnote}\important{\FontElement{footnote}} element. The
font of the footnote mark\IfThisCommonLabelBase{maincls}{}{, however,}
...
\IfThisCommonLabelBase{scrextend}{ %
  ...
  if footnotes are handled by the package, i.\,e., after using
  \Macro{deffootnote}.%
}{} Please don't misuse this element for other purposes, e.\,g., to set the
footnotes ragged right (see also \DescRef{\LabelBase.cmd.raggedfootnote},
...
of the rule may be changed with the element
\FontElement{footnoterule}\important{\FontElement{footnoterule}} using the
...
of font or colour. In order to change the colour, a colour package like
\Package{xcolor}\IndexPackage{xcolor}\important{\Package{xcolor}} must be
loaded as well.%
\EndIndexGroup
\EndIndexGroup
\fi
...
But\textnote{\KOMAScript{} vs. standard classes} it is also possible to change
the justification separately from the rest of the document by redefining
...
\cite{package:ragged2e}).  \IfThisCommonLabelBase{scrextend}{%
  A suitable example can be found in
  \autoref{sec:\ThisCommonFirstLabelBase.footnotes},
