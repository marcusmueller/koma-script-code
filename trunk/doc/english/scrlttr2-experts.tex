...
used to determine whether or not a \PName{pseudo-length} has been defined. The
...
command \Macro{if@plength} with the same meaning.%
% :ODOT
\EndIndexGroup
...
\begin{Declaration}
  ...
  \Macro{foreachkomavarifempty}\Parameter{list of variables}
                               \Parameter{then-code}\Parameter{else-code}
\end{Declaration}
The\ChangedAt{v3.27}{\Class{scrlttr2}\and \Package{scrletter}} command
\Macro{foreachkomavar} executes \PName{command} for each variable of the
comma-separated \PName{list of variables}. The name of the particular variable
will be added as parameter to the \PName{command}.%

The command \Macro{foreachnonemptykomavar} does the same but only for those
variables that are not empty in sense of
\DescRef{\LabelBase.cmd.ifkomavarempty}. Empty variables in the \PName{list of
  variables} are ignored and do not have an effect.

In opposite the command \Macro{foreachemptykomavar} executes \PName{command}
only for variables that are empty in sense of
\DescRef{\LabelBase.cmd.ifkomavarempty}. Accordingly non empty variable do not
have an effect.

Command \Macro{foreachkomavarifempty} is a kind of combination of the two
former commands. It executes the \PName{then-command} only for those variables
of the \PName{list of variables} that are empty and \PName{else-command} for
the non empty variables. Similar to \PName{command} in both cases the name of
