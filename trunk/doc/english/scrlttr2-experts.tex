...
% Date of the translated German file: 2015/02/14
...
Hence, you cannot use the outdated packages
\Package{scrpage2}\IndexPackage{scrpage2} and \Package{scrpage} or less
\KOMAScript-compatible package \Package{fancyhdr}\IndexPackage{fancyhdr} with
\Package{scrletter}. Additionally, internally loading package
\Package{scrlayer-scrpage} automatically activates page style
\Pagestyle{scrheadings}\IndexPagestyle{scrheadings}. This activation changes
...
Package \Package{scrletter} defines the pair or page style \Pagestyle{letter}
...
used by \Package{scrletter}'s \Pagestyle{letter} the same way as for
\Class{scrlttr2}'s \Pagestyle{headings}. The corresponding \Pagestyle{plain}
page style also is similar to \Class{scrlttr2}'s page style
...
With \Package{scrletter} handling of options
\Option{headsepline}\important[i]{\Option{headsepline}\\%
  \Option{footsepline}} and \Option{footsepline} differ a little bit from
