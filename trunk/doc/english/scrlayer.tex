...
Development of this package has not been finished yet. Parts of the package are
...
released for using. Everything, that has not been documented yet, should not be
...
directly give us a position of the layer on the page. For each dimension we
need two reference points, one on the page and one on the layer. The
numerical value than states the distance between the reference these.
...
float environments like tables or figures, if they are allowed to be printed on
...
page is a page, that may itself flow. Non-float pages are not pages
without floats, but pages, that are not float pages. They may contain floats
...
declaration will be ignroed incase a layer with the same layer name already
...
unchanged and will not be reset to the initial default state. Using either
...
\begin{desclist}
  ...
  \entry{\Option{evenpage}}{%
    ...
    does not expect any value. By the default the option is not set and
  ...
  \entry{\Option{nonfloatpage}}{%
    Restricts the layer to pages, that are not float pages. The option does not
...
These commands are valid while output of the layer's contents. So they may be
used inside the \PName{code} of option \Option{contents} of the previously
...
oversized or do not fill the layer completely. Redefinition of the commands to
...
this the layer may not be used any longer. But it does not matter, if the layer
...
\Macro{DeclareNewLayer} or \Macro{ProvideLayer}, but could not be changed using
...
Note that the layer page styles described later in this section, will not need
...
alias newly. In the other case you will get an error.%
...
Note, that the layers of the page style will not be destroyed
